...
describe('global pricing route and navigation', () => {
  ...
  it('shows independent color provider icons in the first pricing table column', () => {
    ...
    expect(viewSource).toContain('uses shared AI logo CDN images')
    const iconSource = readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), '../../components/common/ProviderBrandIcon.vue'), 'utf8')
    expect(iconSource).toContain('brand.iconUrl')
    expect(iconSource).toContain('<img')
    expect(iconSource).toContain('provider-brand-image')
    ...
    expect(viewSource).toContain('global-pricing-card-frame')
    expect(viewSource).toContain('global-pricing-card-body')
    expect(viewSource).toContain(':data-card-label="t(\'globalPricing.columns.brand\')"')
  ...
  it('uses shared material card surfaces on maintained pricing modules', () => {
    ...
    expect(viewSource).toContain('table-wrapper admin-material-surface global-pricing-card-frame')
    expect(viewSource).toContain('global-pricing-summary-row')
    expect(viewSource).toContain('global-pricing-filter-card')
    expect(viewSource).toContain('global-pricing-filter-shell')
    expect(viewSource).toContain('--material-card-surface')
    expect(viewSource).toContain('--material-card-surface: var(--atelier-paper-2);')
    expect(viewSource).not.toContain('--material-card-surface: var(--atelier-material-blue);')
    expect(viewSource).not.toContain('--material-card-surface: var(--atelier-material-dust);')
    expect(viewSource).toContain('background: var(--material-card-surface);')
    expect(viewSource).not.toContain('--material-card-surface: #e8eef8;')
    expect(viewSource).not.toContain('background-size: 28px 28px, 28px 28px, auto;')
    expect(viewSource).not.toContain('.summary-tile:nth-child(')
    expect(viewSource).toContain('background: var(--atelier-dust-soft);')
    expect(viewSource).not.toContain('--atelier-card-accent: var(--atelier-butter);')
    expect(viewSource).toContain('.capability-pill')
