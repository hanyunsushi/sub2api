<template>
  ...
  <div class="table-wrapper">
    ...
    >
      <thead>
        <tr class="border-b border-[var(--anthropic-border)] bg-[var(--anthropic-section)] text-xs font-medium uppercase tracking-wide text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
      ...
      <tbody v-if="loading">
        <tr>
          <td colspan="5" class="py-10 text-center">
            <Icon name="refresh" size="lg" class="inline-block animate-spin text-[var(--anthropic-muted)]" />
          </td>
        </tr>
      </tbody>
      <tbody v-else-if="rows.length === 0">
        <tr>
          <td colspan="5" class="py-12 text-center">
            ...
            <p class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ emptyLabel }}</p>
          </td>
        </tr>
      </tbody>
      ...
      <tbody
        ...
        class="border-b-2 border-[var(--anthropic-border)] last:border-b-0 dark:border-[var(--anthropic-border)]"
      >
        <tr
          v-for="(section, secIdx) in channel.platforms"
          :key="`${channel.name}-${section.platform}`"
          class="transition-colors hover:bg-[var(--anthropic-section)] dark:hover:bg-[var(--anthropic-raised)]/40"
          :class="{ 'border-t border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]': secIdx > 0 }"
        >
          <!-- 渠道名：只在第一行渲染并用 rowspan 纵向合并 -->
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 text-center align-middle font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]"
          >
            {{ channel.name }}
          </td>
          ...
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]"
          >
            <template v-if="channel.description">{{ channel.description }}</template>
            <span v-else class="text-[var(--anthropic-muted)]">-</span>
          </td>
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-col gap-1.5">
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-accent-600 dark:text-accent-500"
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]"
              ...
              <span v-if="section.groups.length === 0" class="text-xs text-[var(--anthropic-muted)]">-</span>
            </div>
          </td>
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-wrap gap-1">
              ...
              />
              <span v-if="section.supported_models.length === 0" class="text-xs text-[var(--anthropic-muted)]">
                {{ noModelsLabel }}
              </span>
            </div>
          </td>
