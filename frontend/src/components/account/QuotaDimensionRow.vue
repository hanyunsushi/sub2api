...
<template>
  <div>
    ...
    <div v-if="quotaNotifyGlobalEnabled" class="flex items-center gap-2 mb-1">
      <span class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] flex-1 min-w-0">{{ label }}</span>
      <span v-if="limit && limit > 0" class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] flex-1 min-w-0">{{ t('admin.accounts.quotaNotify.alert') }}</span>
    </div>
    <label v-else class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] mb-1 block">{{ label }}</label>

    <!-- Input row -->
    <div class="flex items-center gap-2">
      <div :class="['relative', quotaNotifyGlobalEnabled ? 'flex-1 min-w-0' : 'flex-1']">
        <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] text-sm">$</span>
        <input data-testid="account-quota-dimension-row-input-number" :value="limit" @input="onLimitInput" type="number" min="0" step="0.01" class="input pl-6 py-1.5 text-sm" :placeholder="t('admin.accounts.quotaLimitPlaceholder')" />
      </div>
      <QuotaNotifyToggle
    ...
    <div v-if="hasResetMode" class="mt-1 flex items-center gap-2 flex-wrap">
      <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] whitespace-nowrap">{{ t('admin.accounts.quotaResetMode') }}</label>
      <select data-testid="account-quota-dimension-row-select-select" :value="resetMode || 'rolling'" @change="onModeChange" class="input py-1 text-xs w-auto">
      ...
      <template v-if="resetMode === 'fixed'">
        ...
        <template v-if="dim === 'weekly'">
          <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] whitespace-nowrap">{{ t('admin.accounts.quotaWeeklyResetDay') }}</label>
          <select data-testid="account-quota-dimension-row-select-select-2" :value="resetDay ?? 1" @change="emit('update:resetDay', Number(($event.target as HTMLSelectElement).value))" class="input py-1 text-xs w-28">
            <option v-for="d in dayOptions" :key="d.value" :value="d.value">{{ t('admin.accounts.dayOfWeek.' + d.key) }}</option>
          </select>
        </template>
        <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] whitespace-nowrap">{{ t('admin.accounts.quotaResetHour') }}</label>
        <select data-testid="account-quota-dimension-row-select-select-3" :value="resetHour ?? 0" @change="emit('update:resetHour', Number(($event.target as HTMLSelectElement).value))" class="input py-1 text-xs w-24">
        ...
        <template v-if="timezoneOptions && timezoneOptions.length > 0">
          <select data-testid="account-quota-dimension-row-select-select-4" :value="resetTimezone || 'UTC'" @change="emit('update:resetTimezone', ($event.target as HTMLSelectElement).value)" class="input py-1 text-xs w-auto">
            <option v-for="tz in timezoneOptions" :key="tz" :value="tz">{{ tz }} ({{ getTimezoneOffsetLabel(tz) }})</option>
          </select>
        </template>
      </template>
      <span class="text-[11px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
