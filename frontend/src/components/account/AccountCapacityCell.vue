<template>
  <div class="account-capacity-cell flex flex-col gap-0.5">
    <!-- 并发槽位 -->
    <CapacityBadge class="account-capacity-badge-concurrency" :color-class="concurrencyClass" :current="currentConcurrency" :max="account.concurrency">
...
const concurrencyClass = computed(() => {
  ...
  if (current >= max) return semanticBadgeClass('error')
  if (current > 0) return semanticBadgeClass('warning')
...
const windowCostClass = computed(() => {
  ...
  if (current >= limit + reserve) return semanticBadgeClass('error')
  if (current >= limit) return semanticBadgeClass('warning')
  if (current >= limit * 0.8) return semanticBadgeClass('warning')
...
const sessionLimitClass = computed(() => {
  ...
  if (current >= max) return semanticBadgeClass('error')
  if (current >= max * 0.8) return semanticBadgeClass('warning')
  return semanticBadgeClass('success')
})
...
const rpmClass = computed(() => {
  ...
  if (rpmStrategy.value === 'tiered') {
    if (current >= base + buffer) return semanticBadgeClass('error')
    if (current >= base) return semanticBadgeClass('warning')
  } else {
    if (current >= base) return semanticBadgeClass('warning')
  }
  if (current >= base * 0.8) return semanticBadgeClass('warning')
  return semanticBadgeClass('success')
})
