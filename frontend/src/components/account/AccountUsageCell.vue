    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        <div v-if="grokEntitlementLabel" class="mb-0.5">
          <span class="inline-block rounded border border-[var(--anthropic-border-subtle)] bg-transparent px-1.5 py-0.5 text-[10px] font-medium text-[var(--anthropic-fg)]">
            {{ grokEntitlementLabel }}
          </span>
        </div>
        <div v-if="grokLocalUsage" class="mb-0.5 flex items-center">
          <div class="flex items-center gap-1.5 text-[9px] text-[var(--anthropic-muted)]">
            <span class="rounded bg-[var(--anthropic-raised)] px-1.5 py-0.5">
              {{ formatWindowRequests(grokLocalUsage) }} req
            </span>
            <span class="rounded bg-[var(--anthropic-raised)] px-1.5 py-0.5">
              {{ formatWindowTokens(grokLocalUsage) }}
            </span>
            <span class="rounded bg-[var(--anthropic-raised)] px-1.5 py-0.5" :title="t('usage.accountBilled')">
              A ${{ formatWindowCost(grokLocalUsage) }}
            </span>
            <span
              v-if="grokLocalUsage.user_cost != null"
              class="rounded bg-[var(--anthropic-raised)] px-1.5 py-0.5"
        ...
        <div v-if="grokQuotaUnknown" class="text-[10px] text-[var(--anthropic-muted)]">
        ...
        <div v-if="grokQuotaStatusLine" class="text-[10px] text-[var(--anthropic-muted)]">
          {{ grokQuotaStatusLine }}
        </div>
