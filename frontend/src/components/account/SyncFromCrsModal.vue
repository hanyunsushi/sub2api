<template>
  ...
  >
    ...
    >
      <div class="text-sm text-[var(--anthropic-muted)] dark:text-dark-300">
        {{ t('admin.accounts.syncFromCrsDesc') }}
      </div>
      <div
        class="rounded-lg bg-[var(--anthropic-section)] p-3 text-xs text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-dark-400"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-dark-300">
          <input data-testid="account-sync-from-crs-input-form-sync-proxies"
            v-model="form.sync_proxies"
            type="checkbox"
            class="rounded border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        <div class="mb-2 text-sm font-medium text-[var(--anthropic-muted)] dark:text-dark-300">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-[var(--anthropic-muted)]">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-[var(--anthropic-muted)] dark:text-dark-400">
          <div
          ...
          >
            <span
              class="inline-block rounded bg-[var(--anthropic-info-bg)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--anthropic-info)] dark:bg-[var(--anthropic-info-bg)] dark:text-[var(--anthropic-info)]"
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
            ...
            <span class="ml-1 text-xs text-[var(--anthropic-muted)]">({{ previewResult.new_accounts.length }})</span>
          </div>
          <div class="flex gap-2">
            ...
            <button data-testid="account-sync-from-crs-button-select-none"
              type="button"
              class="text-xs text-[var(--anthropic-muted)] hover:text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-lg border border-[var(--anthropic-border)] p-2 dark:border-[var(--anthropic-border)]"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-[var(--anthropic-section)] dark:hover:bg-[var(--anthropic-raised)]/40"
          >
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-[var(--anthropic-muted)] dark:text-dark-300">{{ acc.name }}</span>
          </label>
        </div>
        <div class="mt-1 text-xs text-[var(--anthropic-muted)]">
          {{ t('admin.accounts.crsSelectedCount', { count: selectedIds.size }) }}
        </div>
      </div>

      <!-- Sync options summary -->
      <div class="flex items-center gap-2 text-xs text-[var(--anthropic-muted)] dark:text-dark-400">
        <span>{{ t('admin.accounts.syncProxies') }}:</span>
        <span :class="form.sync_proxies ? 'text-green-600 dark:text-green-400' : 'text-[var(--anthropic-muted)] dark:text-dark-500'">
      ...
      <div
        ...
        class="rounded-lg bg-[var(--anthropic-section)] p-4 text-center text-sm text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-dark-400"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      <div
        class="space-y-2 rounded-xl border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]"
      >
        <div class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
        ...
        <div v-if="errorItems.length" class="mt-2">
          ...
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-[var(--anthropic-section)] p-3 font-mono text-xs dark:bg-[var(--anthropic-section)]"
          >
