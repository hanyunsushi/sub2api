      ...
      >
        <div class="py-1">
          <template v-if="account">
            <button @click="$emit('test', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm">
              <Icon name="play" size="sm" class="text-green-500" :stroke-width="2" />
              {{ t('admin.accounts.testConnection') }}
            </button>
            <button @click="$emit('stats', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm">
              <Icon name="chart" size="sm" class="text-indigo-500" />
              {{ t('admin.accounts.viewStats') }}
            </button>
            <button @click="$emit('schedule', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm">
              <Icon name="clock" size="sm" class="text-orange-500" />
              {{ t('admin.scheduledTests.schedule') }}
            </button>
            <button v-if="canDuplicate" @click="$emit('duplicate', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm">
            ...
            <template v-if="(account.type === 'oauth' || account.type === 'setup-token') && !isShadow">
              <button @click="$emit('reauth', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm text-blue-600">
                <Icon name="link" size="sm" />
                {{ t('admin.accounts.reAuthorize') }}
              </button>
              <button @click="$emit('refresh-token', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm text-purple-600">
                <Icon name="refresh" size="sm" />
                {{ t('admin.accounts.refreshToken') }}
              </button>
            </template>
            <button v-if="isOpenAIOAuthParent" @click="$emit('create-spark-shadow', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm text-amber-600">
              <Icon name="sparkles" size="sm" />
              {{ t('admin.accounts.createSparkShadow') }}
            </button>
            <button v-if="supportsPrivacy" @click="$emit('set-privacy', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm text-emerald-600">
              <Icon name="shield" size="sm" />
              {{ t('admin.accounts.setPrivacy') }}
            </button>
            <div v-if="hasRecoverableState" class="my-1 border-t border-gray-100 dark:border-dark-700"></div>
            <button v-if="hasRecoverableState" @click="$emit('recover-state', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm text-emerald-600">
              <Icon name="sync" size="sm" />
              {{ t('admin.accounts.recoverState') }}
            </button>
            <button v-if="hasQuotaLimit" @click="$emit('reset-quota', account); $emit('close')" class="dropdown-highlight-item flex w-full items-center gap-2 text-sm text-teal-600">
              <Icon name="refresh" size="sm" />
...
const menuStyle = computed(() => ({
  top: `${props.position?.top ?? menuPosition.value.top}px`,
  left: `${props.position?.left ?? menuPosition.value.left}px`,
