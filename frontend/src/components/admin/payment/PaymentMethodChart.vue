      ...
      <div v-for="method in methods" :key="method.type" class="space-y-1">
        <div class="flex items-center justify-between">
          ...
          <div class="space-y-1 text-right">
            <span v-for="[currency, amount] in sortedAmounts(method.amount)" :key="currency" class="block text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
              {{ formatMoney(currency, amount) }}
            </span>
            <span class="ml-2 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              ({{ method.count }})
            </span>
          </div>
        </div>
        <div v-for="[currency, amount] in sortedAmounts(method.amount)" :key="currency" class="flex items-center gap-2">
          <span class="w-10 text-xs text-[var(--anthropic-muted)]">{{ currency }}</span>
          <div class="h-2 flex-1 overflow-hidden rounded-full bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
            <div
...
const colorMap: Record<string, string> = {
  alipay: 'bg-[var(--anthropic-info)]',
  wxpay: 'bg-green-500',
  alipay_direct: 'bg-[var(--anthropic-info)]',
