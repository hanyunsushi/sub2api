<template>
  <div class="usage-stats-grid grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card usage-stat-card p-4 flex items-center gap-3">
      <div class="usage-stat-icon rounded-lg p-2">
        <Icon name="document" size="md" />
      </div>
      <div>
        <p class="text-xs font-medium text-[var(--anthropic-muted)]">{{ t('usage.totalRequests') }}</p>
        <p class="text-xl font-bold">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-[var(--anthropic-muted)]">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="card usage-stat-card p-4 flex items-center gap-3">
      <div class="usage-stat-icon rounded-lg p-2"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" /></svg></div>
      <div>
        <p class="text-xs font-medium text-[var(--anthropic-muted)]">{{ t('usage.totalTokens') }}</p>
        <p class="text-xl font-bold">{{ formatTokens(stats?.total_tokens || 0) }}</p>
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-[var(--anthropic-muted)]">
          ...
          <span class="group relative inline-flex cursor-help items-center gap-0.5" tabindex="0">
            ...
            <svg
              class="h-3.5 w-3.5 text-[var(--anthropic-muted)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
    ...
    <div class="card usage-stat-card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-[var(--anthropic-muted)]">{{ t('usage.totalCost') }}</p>
        <p class="text-xl font-bold text-green-600">
          ${{ (stats?.total_actual_cost || 0).toFixed(4) }}
        </p>
        <p class="text-xs text-[var(--anthropic-muted)]">
    ...
    <div class="card usage-stat-card p-4 flex items-center gap-3">
      ...
      <div><p class="text-xs font-medium text-[var(--anthropic-muted)]">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
