<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-[var(--anthropic-border)] px-3 py-2 text-sm text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:text-[var(--anthropic-muted)]"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-[var(--anthropic-muted)]">#{{ task.id }}</span>
              ...
              <div class="flex flex-wrap items-center gap-4 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    ...
    running: 'bg-[var(--anthropic-info-bg)] text-[var(--anthropic-info)] dark:bg-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]',
    succeeded: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200',
    failed: 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-200',
    canceled: 'bg-[var(--anthropic-raised)] text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]'
  }
  return map[status] || 'bg-[var(--anthropic-raised)] text-[var(--anthropic-muted)]'
}
