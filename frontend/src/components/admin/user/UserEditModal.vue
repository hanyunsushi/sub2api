  ...
  >
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      <div>
        ...
        <input data-testid="admin-user-user-edit-input-form-email" v-model="form.email" type="email" class="input" />
      </div>
      <div>
        <label class="input-label">{{ t('admin.users.password') }}</label>
        <div class="flex gap-2">
          <div class="relative flex-1">
            <input data-testid="admin-user-user-edit-input-form-password" v-model="form.password" type="text" class="input pr-10" :placeholder="t('admin.users.enterNewPassword')" />
            <button data-testid="admin-user-user-edit-button-copy-password" v-if="form.password" type="button" @click="copyPassword" class="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1 transition-colors hover:bg-[var(--anthropic-raised)] dark:hover:bg-[var(--anthropic-raised)]" :class="passwordCopied ? 'text-green-500' : 'text-[var(--anthropic-muted)]'">
          ...
          <button data-testid="admin-user-user-edit-button-generate-password" type="button" @click="generatePassword" class="btn btn-secondary px-3">
      ...
      <div>
        ...
        <input data-testid="admin-user-user-edit-input-form-username" v-model="form.username" type="text" class="input" />
      </div>
      <div>
        <label class="input-label">{{ t('admin.users.form.roleLabel') }}</label>
        <select data-testid="merge-user-edit-modal-form-role-1" v-model="form.role" class="input">
      ...
      <div>
        ...
        <textarea data-testid="admin-user-user-edit-textarea-form-notes" v-model="form.notes" rows="3" class="input"></textarea>
      </div>
      <div>
        <label class="input-label">{{ t('admin.users.columns.concurrency') }}</label>
        <input data-testid="admin-user-user-edit-input-form-concurrency" v-model.number="form.concurrency" type="number" class="input" />
      </div>
      <div>
        <label class="input-label">{{ t('admin.users.form.rpmLimit') }}</label>
        <input data-testid="admin-user-user-edit-input-form-rpm-limit"
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <button data-testid="admin-user-user-edit-button-emit-close" @click="$emit('close')" type="button" class="btn btn-secondary">{{ t('common.cancel') }}</button>
        <button data-testid="admin-user-user-edit-button-submit" type="submit" form="edit-user-form" :disabled="submitting" class="btn btn-primary">
