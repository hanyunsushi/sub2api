<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-lg bg-[var(--atelier-paper-2)] p-5 ">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-[var(--anthropic-page)] shadow-none dark:bg-[var(--anthropic-section)]">
          <span class="text-2xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ user.email }}</p>
          <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>

      <!-- 加载状态 -->
      <div v-if="loading" class="flex justify-center py-12">
        <svg class="h-10 w-10 animate-spin text-[var(--anthropic-fg)]" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            <div class="h-1.5 w-1.5 rounded-full bg-accent-800"></div>
            <h4 class="text-sm font-semibold text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.users.exclusiveGroups') }}</h4>
            <span class="text-xs text-[var(--anthropic-muted)]">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              ...
              :class="config.isSelected
                ? 'border-[var(--anthropic-fg)] bg-[var(--anthropic-section)] shadow-none dark:border-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)]'
                : 'border-[var(--anthropic-border)] bg-[var(--anthropic-page)] hover:border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:hover:border-dark-500'"
            >
              <div class="flex items-center gap-4">
                <!-- 复选框 -->
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input data-testid="admin-user-user-allowed-groups-input-checkbox"
                      type="checkbox"
                      :checked="config.isSelected"
                      @change="toggleExclusiveGroup(config.groupId)"
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-[var(--anthropic-border)] transition-all peer-checked:border-[var(--anthropic-fg)] peer-checked:bg-[var(--atelier-focus)] dark:border-[var(--anthropic-border)] peer-checked:dark:border-[var(--anthropic-fg)]">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ config.groupName }}</span>
                    <span class="inline-flex items-center rounded-full bg-accent-200 px-2 py-0.5 text-xs font-medium text-accent-700 dark:bg-accent-900/40 dark:text-accent-300">
                      {{ t('admin.groups.exclusive') }}
                    </span>
                  </div>
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    <span class="inline-flex items-center gap-1 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                    ...
                    <span class="text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.users.customRate') }}</label>
                  <input data-testid="admin-user-user-allowed-groups-input-number"
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] px-3 py-2 text-sm font-medium transition-colors focus:border-[var(--atelier-focus)] focus:outline-none focus:ring-2 focus:ring-[var(--atelier-focus)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:focus:border-[var(--atelier-focus)]"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              {{ restrictPublicGroups ? t('admin.users.publicGroupsRestricted') : t('admin.users.publicGroups') }}
            </h4>
            <span class="text-xs text-[var(--anthropic-muted)]">({{ publicGroupConfigs.length }})</span>
            <label class="ml-auto flex cursor-pointer items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              <input
                type="checkbox"
                :checked="restrictPublicGroups"
                @change="toggleRestrictPublicGroups"
                class="h-4 w-4 cursor-pointer rounded border-[var(--anthropic-border)] text-[var(--atelier-focus)] focus:ring-[var(--atelier-focus)] dark:border-[var(--anthropic-border)]"
              />
          ...
          <div class="grid gap-3">
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.users.customRate') }}</label>
                  <input data-testid="admin-user-user-allowed-groups-input-number-2"
                    type="number"
                    step="0.001"
                    min="0.001"
                    :value="config.customRate ?? ''"
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
            <svg class="h-8 w-8 text-[var(--anthropic-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <p class="text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <button data-testid="admin-user-user-allowed-groups-button-emit-close" @click="$emit('close')" class="btn btn-secondary px-5">{{ t('common.cancel') }}</button>
        <button data-testid="admin-user-user-allowed-groups-button-handle-save" @click="handleSave" :disabled="submitting || !loaded" class="btn btn-primary px-6">
