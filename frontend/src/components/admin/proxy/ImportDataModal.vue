  ...
  >
    <form id="import-proxy-data-form" class="space-y-4" @submit.prevent="handleImport">
      ...
      <div>
        ...
        <div
          class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-[var(--anthropic-border)] bg-[var(--anthropic-section)] px-4 py-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
        >
          <div class="min-w-0">
            <div class="truncate text-sm text-[var(--anthropic-muted)] dark:text-dark-200">
              {{ fileName || t('admin.proxies.dataImportSelectFile') }}
            </div>
            <div class="text-xs text-[var(--anthropic-muted)] dark:text-dark-400">JSON (.json)</div>
          </div>
          <button data-testid="admin-proxy-import-data-button-open-file-picker" type="button" class="btn btn-secondary shrink-0" @click="openFilePicker">
            {{ t('common.chooseFile') }}
          </button>
        </div>
        <input data-testid="admin-proxy-import-data-input-file-input"
          ref="fileInput"
          type="file"
          class="hidden"
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <button data-testid="admin-proxy-import-data-button-handle-close" class="btn btn-secondary" type="button" :disabled="importing" @click="handleClose">
          {{ t('common.cancel') }}
        </button>
        <button data-testid="admin-proxy-import-data-button-submit"
