    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.settings.payment.paymentMode') }}</span>
          <div class="flex gap-1.5">
            <button data-testid="payment-payment-provider-button-payment-mode-mode-value"
              ...
              :class="[
                'anthropic-segmented-option px-2.5 py-1 text-xs font-medium transition-all',
                form.payment_mode === mode.value
                  ? 'anthropic-segmented-option-active'
                  : '',
              ]"
            >{{ mode.label }}</button>
          </div>
        ...
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div class="border-t border-[var(--anthropic-border)] pt-4 dark:border-[var(--anthropic-border)]">
        <div class="mb-3 flex items-center gap-2">
          <h4 class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
        ...
        <p v-if="paymentGuide" class="mb-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-[var(--anthropic-muted)]">({{ t('common.optional') }})</span>
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button data-testid="payment-payment-provider-button-key-visible-fields-field-key"
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-[var(--anthropic-muted)] hover:text-[var(--anthropic-muted)] dark:hover:text-gray-300"
              >
            ...
            />
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              {{ t(field.hintKey) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] px-3 text-xs text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] px-3 text-xs text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>

        <!-- 服务商 Webhook 提示 -->
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg border border-[var(--anthropic-info-border)] bg-[var(--anthropic-info-bg)] p-3 dark:border-[var(--anthropic-info-border)] dark:bg-[var(--anthropic-info-bg)]">
          <p class="text-xs text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]">
            {{ t(providerWebhookHint) }}
          </p>
          <code class="mt-1 block break-all rounded bg-[var(--anthropic-info-bg)] px-2 py-1 text-xs text-[var(--anthropic-info)] dark:bg-[var(--anthropic-info-bg)] dark:text-[var(--anthropic-info)]">
            {{ providerWebhookUrl }}
          </code>
          <p v-if="form.provider_key === 'stripe'" class="mt-2 text-xs leading-relaxed text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]">
      ...
      <div v-if="limitableTypes.length" class="border-t border-[var(--anthropic-border)] pt-4 dark:border-[var(--anthropic-border)]">
        <button data-testid="payment-payment-provider-button-limits-expanded-limits-expanded" type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-[var(--anthropic-muted)] transition-transform', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          ...
          >
            <p class="mb-2 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.settings.payment.limitSingleMin') }}</label>
              ...
              <div>
                <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.settings.payment.limitSingleMax') }}</label>
              ...
              <div>
                <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.settings.payment.limitDaily') }}</label>
          ...
          <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
