<template>
  <div class="space-y-6">
    <section
      ...
      class="card overflow-hidden border border-[var(--atelier-line)] bg-[var(--atelier-paper-2)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
    >
      <div class="px-6 py-6 md:px-8">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
          <div
            class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[var(--atelier-ink)] text-2xl font-bold text-white shadow-none"
          >
            <img
              v-if="avatarUrl"
              :src="avatarUrl"
          ...
          <div class="min-w-0 flex-1 space-y-5">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                <h2 class="truncate text-2xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
                  {{ displayName }}
                </h2>
              ...
              <div class="space-y-1">
                <p class="truncate text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                  {{ primaryEmailDisplay }}
                </p>
                <div
                  v-if="sourceHints.length"
                  class="flex flex-wrap gap-2 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]"
                >
                  <span
                    v-for="hint in sourceHints"
                    :key="hint.key"
                    class="inline-flex items-center gap-1 rounded-lg bg-[var(--atelier-paper)] px-3 py-1 ring-1 ring-[var(--atelier-line)] dark:bg-[var(--anthropic-section)] dark:ring-dark-700"
                  >
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              <div
                ...
                class="rounded-lg bg-[var(--atelier-paper)] px-4 py-3 shadow-none ring-1 ring-[var(--atelier-line)] dark:bg-[var(--anthropic-section)] dark:ring-dark-700"
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                  {{ t('profile.accountBalance') }}
                </p>
                <p class="mt-1 text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
    ...
    <div class="space-y-6">
      <div data-testid="profile-main-column" class="space-y-6">
        <section
          ...
          class="card border border-[var(--atelier-line)] bg-[var(--atelier-paper-2)] p-6 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
        >
          <div class="mb-5 flex items-start justify-between gap-4">
            <div>
              <h3 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
          ...
          <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
            <div class="rounded-lg border border-[var(--atelier-line)] bg-[var(--atelier-paper)] p-5 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <ProfileAvatarCard
                :user="user"
                embedded
              />
      ...
      <div data-testid="profile-side-column" class="space-y-6">
        <section
          v-if="sourceHints.length"
          class="card border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-6 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
        >
          <h3 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
          ...
          <div class="mt-5 grid gap-3">
            <div
              v-for="hint in sourceHints"
              :key="hint.key"
              class="flex items-start gap-3 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] px-4 py-3 text-sm text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]"
            >
              <Icon name="link" size="sm" class="mt-0.5 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]" />
              <span>{{ hint.text }}</span>
            </div>
          </div>
        </section>
