  ...
  <div class="card">
    ...
    <div class="px-6 py-6">
      ...
      <div v-else-if="status && !status.feature_enabled" class="flex items-center gap-4 py-4">
        <div class="flex-shrink-0 rounded-full bg-[var(--anthropic-raised)] p-3 dark:bg-[var(--anthropic-section)]">
          <svg class="h-6 w-6 text-[var(--anthropic-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
          </svg>
        </div>
        <div>
          <p class="font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
      ...
      <div v-else-if="status?.enabled" class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
              {{ t('profile.totp.enabled') }}
            </p>
            <p v-if="status.enabled_at" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
