...
const statusClass = computed(() => {
  ...
  if (current >= max && max > 0) {
    return 'border-[color-mix(in_srgb,var(--anthropic-error)_32%,transparent)] text-[var(--anthropic-error)]'
  }
  // In use: yellow
  if (current > 0) {
    return 'border-[color-mix(in_srgb,var(--anthropic-warning)_32%,transparent)] text-[var(--anthropic-warning)]'
  }
  // Idle: gray
  return 'border-[var(--anthropic-border)] text-[var(--anthropic-muted)]'
})
</script>
