<template>
      ...
      <div v-else-if="attr.type === 'multi_select'" class="space-y-2">
        ...
        >
          <input data-testid="user-user-attribute-form-input-checkbox"
            ...
            class="h-4 w-4 rounded border-[var(--anthropic-border)] text-[var(--anthropic-fg)]"
          />
          <span class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ opt.label }}</span>
        </label>
      </div>
  ...
  <div v-else-if="loading" class="flex justify-center py-4">
    <svg class="h-5 w-5 animate-spin text-[var(--anthropic-muted)]" fill="none" viewBox="0 0 24 24">
