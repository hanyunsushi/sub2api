...
<template>
  ...
  >
    ...
    <slot name="trigger">
      <svg
        class="h-4 w-4 cursor-help text-[var(--anthropic-muted)] transition-colors hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-muted)] dark:hover:text-[var(--anthropic-fg)]"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
    ...
    <Teleport to="body">
      ...
      <div
        ...
        role="tooltip"
        :class="[
          'fixed z-[99999] -translate-x-1/2 -translate-y-full rounded-lg bg-gray-900 p-3 text-xs leading-relaxed text-white shadow-none ring-1 ring-[var(--anthropic-border-subtle)] selection:bg-primary-200 selection:text-gray-900 before:absolute before:inset-x-0 before:top-full before:h-3 dark:bg-[var(--anthropic-section)] dark:selection:bg-primary-200 dark:selection:text-gray-900',
          props.widthClass,
        ]"
      ...
      >
        <button data-testid="common-help-tooltip-button-close-tooltip"
          ...
          type="button"
          class="absolute right-1.5 top-1.5 rounded p-1 text-gray-300 transition-colors hover:bg-[var(--anthropic-page)] hover:text-white"
          aria-label="Close"
        ...
        <slot>{{ content }}</slot>
        <div class="absolute -bottom-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-gray-900 dark:bg-[var(--anthropic-section)]"></div>
      </div>
    </Teleport>
  </div>
</template>
