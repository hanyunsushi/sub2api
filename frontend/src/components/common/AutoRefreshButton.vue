<template>
  <div class="relative" ref="dropdownRef">
    <button data-testid="common-auto-refresh-button-button-show-dropdown-show-dropdown"
      ...
      class="auto-refresh-button inline-flex items-center gap-1.5 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] px-2.5 py-1.5 text-xs font-medium text-[var(--anthropic-muted)] shadow-none transition-colors hover:bg-[var(--anthropic-section)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]"
    ...
    <div
      v-if="showDropdown"
      class="auto-refresh-button-menu absolute right-0 z-20 mt-1 w-44 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
    >
      <div class="p-1.5">
        <button data-testid="common-auto-refresh-button-button-emit-update-enabled-enabled"
          @click="$emit('update:enabled', !enabled)"
          class="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-sm text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-raised)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]"
        >
          <span>{{ t('common.autoRefresh.enable') }}</span>
          <svg v-if="enabled" class="h-4 w-4 text-[var(--anthropic-fg)]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        >
          ...
          <svg v-if="intervalSeconds === sec" class="h-4 w-4 text-[var(--anthropic-fg)]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" />
          </svg>
        </button>
      </div>
