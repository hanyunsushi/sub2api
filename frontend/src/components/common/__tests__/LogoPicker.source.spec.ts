...
describe('LogoPicker preset gallery contract', () => {
  it('offers the shared CDN AI logo gallery as direct selectable presets', () => {
    ...
    expect(logoPickerSource).toContain('deleteCustomAILogoPreset')
    ...
    expect(logoPickerSource).toContain('@click.stop="deletePreset(preset.url)"')
  ...
  it('shows delete controls only for server-backed custom AI logo presets', () => {
    expect(logoPickerSource).toContain('isCustomPreset(preset.url)')
    expect(logoPickerSource).toContain('logo-picker-preset-delete')
    expect(logoPickerSource).toContain("aria-label=\"Delete custom logo\"")
    expect(logoPickerSource).toContain('function deletePreset(url: string)')
    expect(logoPickerSource).toContain("emit('update:modelValue', '')")
    expect(logoPickerSource).toContain("Custom logo")
  })
