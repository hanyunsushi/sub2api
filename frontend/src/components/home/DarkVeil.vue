<template>
  <canvas ref="canvasRef" class="darkveil-canvas" aria-hidden="true"></canvas>
</template>
...
const maxDrawEdge = 1280
const maxDrawPixels = 1_200_000

function updateUniforms(time = 0) {
  if (!program) return
...
function resize() {
  ...
  const height = Math.max(1, parent?.clientHeight || canvas.clientHeight || 480)
  const scaledWidth = width * props.resolutionScale
  const scaledHeight = height * props.resolutionScale
  const drawScale = Math.min(
    1,
    maxDrawEdge / Math.max(scaledWidth, scaledHeight),
    Math.sqrt(maxDrawPixels / (scaledWidth * scaledHeight)),
  )
  const drawWidth = Math.max(1, Math.round(scaledWidth * drawScale))
  const drawHeight = Math.max(1, Math.round(scaledHeight * drawScale))
