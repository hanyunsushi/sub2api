<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-[var(--anthropic-page)] p-6 shadow-none transition-all dark:bg-[var(--anthropic-section)]">
        <!-- Header -->
        <div class="mb-6 text-center">
          <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[var(--anthropic-section)] dark:bg-[var(--anthropic-section)]">
            <svg class="h-6 w-6 text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
          </div>
          <h3 class="mt-4 text-xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
          ...
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
            {{ userEmailMasked }}
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <!-- Loading indicator -->
          <div v-if="verifying" class="mt-3 flex items-center justify-center gap-2 text-sm text-[var(--anthropic-muted)]">
            <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-[var(--anthropic-fg)]"></div>
            {{ t('common.verifying') }}
          </div>
        </div>
