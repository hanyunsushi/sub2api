...
describe('AppSidebar scroll position', () => {
  it('preserves the scroll position across route-click render and focus updates', () => {
    ...
    expect(componentSource).toContain('function scheduleSidebarScrollCommit')
    expect(componentSource).toContain('function cancelSidebarScrollCommitSchedule')
    expect(componentSource).toContain('function flushSidebarScrollCommit')
    ...
    expect(componentSource).toContain('sidebarScrollCommitFrame')
    expect(componentSource).not.toContain('let preservedSidebarScrollTop')
    expect(componentSource).toContain('captureSidebarScroll()')
    expect(componentSource).toContain('cancelSidebarScrollCommitSchedule()')
    expect(styleSource).not.toMatch(/\\.sidebar-nav,\\n\\s*\\.modal-body/)
  })

  it('keeps reactive store writes out of the scroll event hot path', () => {
    const scrollHandlerMatch = componentSource.match(/function handleSidebarNavScroll\(\) \{[\s\S]*?\n\}/)
    const schedulerMatch = componentSource.match(/function scheduleSidebarScrollCommit\(\) \{[\s\S]*?\n\}/)

    expect(scrollHandlerMatch).not.toBeNull()
    expect(schedulerMatch).not.toBeNull()
    expect(scrollHandlerMatch?.[0]).toContain('latestSidebarScrollTop = sidebarNavRef.value.scrollTop')
    expect(scrollHandlerMatch?.[0]).toContain('scheduleSidebarScrollCommit()')
    expect(scrollHandlerMatch?.[0]).not.toContain('appStore.setSidebarNavScrollTop')
    expect(schedulerMatch?.[0]).toContain('requestAnimationFrame')
    expect(schedulerMatch?.[0]).toContain('commitSidebarScrollTop()')
