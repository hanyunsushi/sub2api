...
const materialSystemBlock = styleSource.slice(
  ...
  styleSource.indexOf('Anthropic theme — editorial component pass')
)
...
describe('right-side material surfaces', () => {
  it('ships a single atelier component material pass for all maintained component families', () => {
    ...
    expect(styleSource).toContain('--atelier-material-butter: color-mix(in srgb, var(--atelier-butter) 18%, var(--atelier-paper));')
    expect(styleSource).toContain('--atelier-material-butter-strong: color-mix(in srgb, var(--atelier-butter) 28%, var(--atelier-paper));')
    ...
    expect(materialSystemBlock).not.toContain(blockedBackdropFilter)
    expect(materialSystemBlock).not.toContain(blockedWebkitBackdropFilter)
  ...
  it('keeps the console paper palette even when the global dark class is present', () => {
    ...
    expect(styleSource).toContain('--atelier-material-shadow: 0 4px 18px rgba(20, 20, 19, 0.025);')
    expect(styleSource).toContain('[class~="bg-white"]')
    expect(styleSource).toContain('[class~="rounded-lg"]')
    expect(materialSystemBlock).toContain('.dark .app-layout-shell :where(.card, .paper-card, .paper-surface, .stat-card, .summary-tile, .admin-material-surface, .table-container, .table-wrapper, .table-scroll-container, .modal-content, .dialog-container, .toast, .codex-panel, .codex-account-card, .codex-topbar, .codex-side, .layout-section-fixed, .settings-tabs-shell)')
    expect(materialSystemBlock).toContain('background: var(--atelier-card-surface, var(--atelier-paper-2)) !important;')
    expect(materialSystemBlock).not.toContain('var(--atelier-material-grid-dark),')
    expect(materialSystemBlock).toContain('color: var(--atelier-ink) !important;')
    ...
    expect(dataTableSource).toContain('background-color: var(--atelier-dark);')
  ...
  it('themes old white and gray component internals with the full atelier palette', () => {
    ...
    expect(materialSystemBlock).not.toContain('color: var(--atelier-butter-dark);')
    expect(materialSystemBlock).not.toContain('color: var(--atelier-butter-dark) !important;')
    expect(materialSystemBlock).not.toContain('background: var(--atelier-material-butter) !important;')
