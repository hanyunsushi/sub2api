...
describe('AppLayout route transition', () => {
  it('marks only the right-side page content as the transition target', () => {
    ...
    expect(componentSource).toContain('opacity 0.6s cubic-bezier(0.16, 1, 0.3, 1)')
    expect(componentSource).toContain('will-change: opacity, transform')
    expect(componentSource).toContain('animation: app-route-page-enter 0.6s cubic-bezier(0.16, 1, 0.3, 1) both')
    expect(componentSource).toContain('@keyframes app-route-page-enter')
    expect(componentSource).toContain('translateY(16px) scale(0.985)')
  })
})
