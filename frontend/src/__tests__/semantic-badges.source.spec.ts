...
describe('semantic badges', () => {
  ...
  it('keeps API key group and status badges on official semantic colors', () => {
    ...
    expect(groupBadgeSource).toContain('group-token')
    expect(groupBadgeSource).toContain('group-token--openai')
    expect(groupBadgeSource).toContain('group-token--anthropic')
    expect(groupBadgeSource).toContain('group-token--gemini')
    expect(groupBadgeSource).not.toContain("'group-badge")
    expect(groupBadgeSource).not.toContain('group-badge--')
    expect(keysSource).toContain("'api-key-status-badge'")

    expect(styleSource).toContain('.user-keys-atelier .group-token--openai')
    ...
    expect(styleSource).not.toContain('.user-keys-atelier .group-token--openai {\n  --api-key-group-color: var(--atelier')
  })
})
