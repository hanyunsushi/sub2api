...
describe('Anthropic hover and overlay taxonomy', () => {
  ...
  it('keeps authentication pages on the same button, input, checkbox, and logo contract', () => {
    ...
    expect(authLayoutSource).toContain('bg-transparent shadow-none ring-0')
    expect(authLayoutSource).toContain('border: 0;')
    expect(authLayoutSource).toContain('box-shadow: none;')
    expect(loginViewSource).toContain('class="btn btn-primary w-full"')
    expect(registerViewSource).toContain('class="btn btn-primary w-full"')
    ...
    expect(authInputBlock).toContain('background: var(--anthropic-page) !important;')
    expect(authInputBlock).toContain('box-shadow: none !important;')
    const authInputMouseFocusBlock = cssBlock(targetedRepairSource, '.auth-ascii-shell :where(.input, input[type="text"], input[type="search"], input[type="email"], input[type="password"], input[type="number"], input[type="date"], input[type="tel"], textarea, select):where(:focus):not(:focus-visible)')
    ...
    expect(authInputKeyboardFocusBlock).toContain('outline: 2px solid var(--anthropic-border-hover) !important;')

    const authCheckboxBlock = cssBlock(targetedRepairSource, '.auth-ascii-shell :where(input[type="checkbox"]:not(.sr-only):not(.peer), .anthropic-checkbox)')
    ...
    const authCheckboxCheckedBlock = cssBlock(targetedRepairSource, '.auth-ascii-shell :where(input[type="checkbox"]:not(.sr-only):not(.peer), .anthropic-checkbox):where(:checked)')
