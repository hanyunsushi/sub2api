...
describe('Anthropic hover and overlay taxonomy', () => {
  ...
  it('keeps filter text controls out of the button/focus-blue taxonomy', () => {
    ...
    expect(dashboardTextControlKeyboardBlock).toContain('outline: 2px solid var(--anthropic-focus, var(--atelier-focus)) !important;')
  ...
  it('keeps table filter sections single-framed and management pages on the shared toolbar contract', () => {
    ...
    expect(subscriptionsViewSource).toContain('class="table-filter-shell subscriptions-filter-shell flex flex-wrap items-center gap-2"')
    expect(subscriptionsViewSource).toContain('class="table-filter-left subscriptions-filter-left flex min-w-0 flex-1 flex-wrap items-center gap-2"')
    ...
    expect(subscriptionsViewSource).toContain('class="table-filter-actions subscriptions-filter-actions flex w-full flex-shrink-0 flex-wrap items-center justify-end gap-2 lg:w-auto"')
    ...
    expect(subscriptionsShellBlock).toContain('row-gap: 0.375rem !important;')
    expect(subscriptionsShellBlock).toContain('column-gap: 0.5rem !important;')
    ...
    expect(subscriptionsGroupsBlock).toContain('row-gap: 0.375rem !important;')
    expect(subscriptionsGroupsBlock).toContain('column-gap: 0.5rem !important;')
    const subscriptionsSearchBlock = cssBlock(targetedRepairSource, '#app .app-layout-content .table-page-filter-section > .subscriptions-filter-shell .subscriptions-filter-search')
    expect(subscriptionsSearchBlock).toContain('width: 11rem !important;')
    expect(subscriptionsSearchBlock).toContain('flex: 0 0 11rem !important;')
    const subscriptionsStatusPlatformBlock = cssBlock(targetedRepairSource, '#app .app-layout-content .table-page-filter-section > .subscriptions-filter-shell :where(.subscriptions-filter-status, .subscriptions-filter-platform)')
    expect(subscriptionsStatusPlatformBlock).toContain('width: 6.75rem !important;')
    expect(subscriptionsStatusPlatformBlock).toContain('flex: 0 0 6.75rem !important;')
    const subscriptionsGroupBlock = cssBlock(targetedRepairSource, '#app .app-layout-content .table-page-filter-section > .subscriptions-filter-shell .subscriptions-filter-group')
    expect(subscriptionsGroupBlock).toContain('width: 7.75rem !important;')
    expect(subscriptionsGroupBlock).toContain('flex: 0 0 7.75rem !important;')
    ...
    expect(globalPricingAnthropicPillBlock).toContain('--provider-pill-border: var(--anthropic-warning-border);')
    expect(globalPricingAnthropicPillBlock).toContain('--provider-pill-bg: var(--anthropic-warning-bg);')
    expect(globalPricingAnthropicPillBlock).toContain('--provider-pill-fg: var(--anthropic-warning);')
  ...
  it('keeps the account bulk actions bar frameless and avoids duplicate filtered edit entry points', () => {
    ...
    expect(accountBulkActionsBarSource).toContain('class="btn btn-primary btn-sm account-bulk-primary-action" data-testid="account-bulk-edit-selected"')
    expect(accountBulkActionsBarSource).toContain('class="btn btn-primary btn-sm account-bulk-primary-action" data-testid="account-bulk-edit-filtered"')
