...
describe('console atelier palette restyle', () => {
  ...
  it('uses uniform paper-2 modules and dotted rules while preserving existing component class hooks', () => {
    ...
    expect(dashboardFilterBlock).toContain('background: var(--atelier-ink) !important;')
  ...
  it('adapts the ops monitoring page to the dashboard module style without moving existing modules', () => {
    ...
    expect(styleSource).toContain('background: var(--atelier-ink) !important;')
    expect(styleSource).toContain('--atelier-material-2: var(--atelier-ui-hover-surface);')
    expect(styleSource).toContain('background-color: var(--atelier-ui-hover-surface) !important;')
  ...
  it('themes console tables, inputs, and teleported menus as opaque atelier paper surfaces', () => {
    ...
    expect(materialSystemBlock).toContain('Non-dashboard/ops filter bars are unified ink blocks with perimeter stitching.')
  ...
  it('keeps the latest filter split, account cards, quota bars, and settings nav palette hooks', () => {
    ...
    expect(cssBlock(materialSystemBlock, '#app .app-layout-content .settings-tab')).toContain('border-radius: 0 !important;')
    ...
    expect(cssBlock(materialSystemBlock, '#app .app-layout-content .settings-tab::before')).toContain('border-radius: 0 !important;')
  ...
  it('keeps dashboard and ops filter bars black and removes non-sidebar butter hover highlights', () => {
    ...
    expect(styleSource).toContain('--select-menu-selected-text: var(--atelier-white);')
    ...
    expect(styleSource).toContain('--date-picker-active-text: var(--atelier-white);')
  ...
  it('wraps the current visual language as the Newspaper theme without moving layout', () => {
    ...
    expect(themeSwitcherSource).toContain('panel-class="theme-switcher-menu w-44 overflow-hidden"')
    expect(themeSwitcherSource).not.toContain('dark:bg-dark-800')
    expect(appearanceThemeSource).toContain("export type AppearanceThemeId = 'newspaper'")
