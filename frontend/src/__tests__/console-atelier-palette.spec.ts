...
describe('console atelier palette restyle', () => {
  ...
  it('themes console tables, inputs, and teleported menus as opaque atelier paper surfaces', () => {
    ...
    expect(materialSystemBlock).toContain('var(--atelier-filter-stitch-horizontal) top left / 100% 1px no-repeat')
    expect(materialSystemBlock).toContain('var(--atelier-filter-stitch-vertical) top right / 1px 100% no-repeat')
    ...
    expect(materialSystemBlock.indexOf('#app .app-layout-content .ops-dashboard-atelier :where(.ops-alert-card, .ops-log-card):hover')).toBeGreaterThan(
      materialSystemBlock.indexOf('#app .app-layout-content :where(.admin-dashboard-atelier, .ops-dashboard-atelier, .admin-usage-atelier, .user-usage-atelier) :where(.card, .paper-card, .stat-card, .summary-tile, .admin-material-surface, .ops-monitor-panel, .ops-metric-card, .ops-system-card):hover')
    )
