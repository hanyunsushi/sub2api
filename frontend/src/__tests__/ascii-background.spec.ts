...
describe('Klein blue ASCII background', () => {
  ...
  it('defines the fixed canvas, grid, and dot texture layers without relying on CSS keyframes', () => {
    ...
    expect(css).toContain('rgba(243, 239, 229, 0.88)')
    ...
    expect(css).toContain('radial-gradient(circle at 18% 0%, rgba(0, 47, 167, 0.075), transparent 26rem)')
    expect(css).toContain('radial-gradient(circle at 86% 16%, rgba(79, 106, 140, 0.11), transparent 24rem)')
