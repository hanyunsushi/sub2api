...
describe('Klein blue ASCII background', () => {
  ...
  it('defines the fixed canvas, grid, and dot texture layers without relying on CSS keyframes', () => {
    ...
    expect(css).toContain('radial-gradient(circle at 84% 9%, rgba(199, 154, 58, 0.1), transparent 24rem)')
    expect(css).toContain('var(--atelier-paper)')
    expect(css).toContain('background-size: auto, auto, 32px 32px, 32px 32px, auto')
    ...
    expect(css).toContain('radial-gradient(circle at 16% 0%, rgba(0, 47, 167, 0.075), transparent 26rem)')
    expect(css).toContain('radial-gradient(circle at 88% 6%, rgba(199, 154, 58, 0.08), transparent 24rem)')
