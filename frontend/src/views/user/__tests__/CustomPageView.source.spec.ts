...
describe('CustomPageView iframe source', () => {
  it('keeps iframe custom menu pages full-height on mobile instead of collapsing into a tiny window', () => {
    ...
    expect(source).toContain('height: max(28rem, calc(100dvh - 64px - 2rem));')
    expect(source).toContain('min-height: max(28rem, calc(100dvh - 64px - 2rem));')
    ...
    expect(source).toContain('min-height: inherit;')
  })
})
