<template>
  <AppLayout>
    <TablePageLayout class="user-usage-atelier" scroll-mode="page">
      <template #actions>
        <div class="usage-stats-grid grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div class="flex items-center gap-3">
            <div class="usage-stat-icon rounded-lg bg-[var(--anthropic-info-bg)] p-2 dark:bg-[var(--anthropic-info-bg)]">
              <Icon name="document" size="md" class="text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]" />
            </div>
            <div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                ...
                <span> · </span>
                <span class="text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]">{{ t('usage.cacheHit') }} {{ formatTokens(usageStats?.total_cache_read_tokens || 0) }}</span>
                <span> · </span>
              ...
              <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                ...
                <template v-if="cacheStats.totalInput > 0">
                  <span class="text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]">{{ formatTokens(cacheStats.cacheRead) }}</span>
                  <span class="text-[var(--anthropic-muted)]">/</span>
                  <span class="text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ formatTokens(cacheStats.totalInput) }}</span>
        ...
        <div class="card usage-stat-card p-4">
          <div class="flex items-center gap-3">
            <div class="usage-stat-icon rounded-lg bg-accent-200 p-2 dark:bg-accent-900/30">
              <Icon name="clock" size="md" class="text-accent-600 dark:text-accent-500" />
            </div>
            <div>
              <p class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                {{ t('usage.avgDuration') }}
              ...
              <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('usage.perRequest') }}</p>
            </div>
          </div>
        </div>
        </div>
      ...
      <template #filters>
        <div class="usage-filter-shell table-filter-shell flex flex-wrap items-end gap-3">
          <div class="usage-filter-left table-filter-left flex flex-1 flex-wrap items-end gap-3">
            <template v-if="activeTab === 'errors'">
              <div class="min-w-[180px]">
                ...
                <Select variant="text-control" v-model="errorFilter.api_key_id" :options="errorKeyOptions" @change="applyErrorFilters" />
              </div>
              <div class="min-w-[180px]">
              ...
              <div class="min-w-[160px]">
                ...
                <Select variant="text-control" v-model="errorFilter.category" :options="errorCategoryOptions" @change="applyErrorFilters" />
              </div>
          ...
          <div class="usage-filter-actions table-filter-actions ml-auto flex items-center gap-3">
            ...
            <div class="relative" ref="columnDropdownRef">
              <button
                data-testid="user-usage-button-column-settings"
                type="button"
              ...
              >
                ...
                >
                  <span>{{ col.label }}</span>
                  <Icon v-if="isCurrentColumnVisible(col.key)" name="check" size="sm" class="text-[var(--anthropic-fg)]" />
                </button>
              </div>
            </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-endpoint="{ row }">
            <span class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] block max-w-[320px] whitespace-normal break-all">
            ...
            <div v-else class="flex items-center gap-1.5">
              <div class="space-y-1.5 text-sm">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <!-- Output -->
                  <div class="inline-flex items-center gap-1">
                    <Icon name="arrowUp" size="sm" class="text-accent-600" />
                    <span class="font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{
                ...
                >
                  ...
                  <div v-if="row.cache_read_tokens > 0" class="inline-flex items-center gap-1">
                    <Icon name="inbox" size="sm" class="text-[var(--anthropic-info)]" />
                    <span class="font-medium text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]">{{
              ...
              >
                <div
                  class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-[var(--anthropic-raised)] transition-colors dark:bg-[var(--anthropic-section)]"
                >
                  <Icon
                    name="infoCircle"
                    size="xs"
          ...
          <template #cell-user_agent="{ row }">
            <span v-if="row.user_agent" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] block max-w-[320px] whitespace-normal break-all" :title="row.user_agent">{{ formatUserAgent(row.user_agent) }}</span>
            <span v-else class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">-</span>
          </template>

          <template #cell-ip_address="{ row }">
            <span class="font-mono text-sm text-[var(--anthropic-muted)]">{{ row.ip_address || '-' }}</span>
          </template>
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
      >
      ...
      >
        <div class="space-y-1.5">
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-[var(--anthropic-info)]"
          ...
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            <span class="text-[var(--anthropic-muted)]">{{ t('usage.billed') }}</span>
