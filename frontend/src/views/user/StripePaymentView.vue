<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-[var(--anthropic-section)] dark:bg-[var(--anthropic-section)]' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('payment.stripeLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ initError }}</p>
        <button data-testid="user-stripe-payment-button-push-purchase" class="btn btn-primary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        <div v-if="order" class="card overflow-hidden">
          <div class="bg-[#141413] px-6 py-6 text-center">
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('payment.qr.scanWxpay') }}</p>
              ...
              <p class="text-center text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <button data-testid="user-stripe-payment-button-handle-generic-pay" class="btn btn-stripe mt-6 w-full py-3 text-base" :disabled="stripeSubmitting || !stripeReady" @click="handleGenericPay">
          ...
          <div class="text-center">
            <button data-testid="user-stripe-payment-button-push-purchase-2" class="btn btn-secondary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
          </div>
        </template>
        ...
        <div v-if="stripeError && !showPaymentElement" class="card p-4">
          ...
          <button data-testid="user-stripe-payment-button-push-purchase-3" class="btn btn-secondary mt-3 w-full" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
        </div>
      </template>
    </div>
  </component>
