<template>
  <div class="relative flex min-h-screen flex-col bg-[var(--anthropic-section)] dark:bg-dark-950">
    <!-- Header (same pattern as HomeView) -->
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          <div class="h-10 w-10 overflow-hidden rounded-xl shadow-none">
            <img :src="siteLogo || '/logo.png'" alt="Logo" class="h-full w-full object-contain" />
          </div>
          <span class="text-lg font-semibold tracking-normal text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ siteName }}</span>
        </router-link>
        <div class="flex items-center gap-3">
          <LocaleSwitcher />
          <a data-testid="key-usage-link-a"
            v-if="docUrl"
            :href="docUrl"
            target="_blank"
            rel="noopener noreferrer"
            class="rounded-lg p-2 text-[var(--anthropic-muted)] transition-colors hover:bg-[var(--anthropic-raised)] hover:text-[var(--anthropic-muted)] dark:text-dark-400 dark:hover:bg-[var(--anthropic-raised)] dark:hover:text-white"
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-normal mb-3 text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-[var(--anthropic-muted)] dark:text-dark-400 text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--anthropic-muted)] dark:text-dark-500">
              <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
              </svg>
            </div>
            <input data-testid="key-usage-input-api-key"
              v-model="apiKey"
              :type="keyVisible ? 'text' : 'password'"
              :placeholder="t('keyUsage.placeholder')"
              class="input-ring w-full h-12 pl-12 pr-12 rounded-xl border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] text-sm text-[var(--anthropic-fg)] placeholder:text-[var(--anthropic-muted)] transition-all dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)] dark:placeholder:text-dark-500"
              @keydown.enter="queryKey"
            />
            <button data-testid="key-usage-button-key-visible-key-visible"
              @click="keyVisible = !keyVisible"
              class="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--anthropic-muted)] hover:text-[var(--anthropic-muted)] dark:text-dark-500 dark:hover:text-white transition-colors"
            >
          ...
          <button data-testid="key-usage-button-query-key"
            @click="queryKey"
            :disabled="isQuerying"
            class="h-12 px-7 rounded-xl bg-[var(--anthropic-fg)] hover:bg-[var(--anthropic-fg-hover)] text-white font-medium text-sm transition-all active:scale-[0.97] flex items-center gap-2 whitespace-nowrap disabled:opacity-60"
          >
        ...
        <p class="text-xs text-[var(--anthropic-muted)] dark:text-dark-500 mt-3 text-center">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.dateRange') }}</span>
            <button data-testid="key-usage-button-set-date-range-range-key"
              ...
              :class="currentRange === range.key
                ...
                : 'border-[var(--anthropic-border)] bg-[var(--anthropic-page)] text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-dark-200 hover:border-[var(--anthropic-fg)] dark:hover:border-dark-600'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input data-testid="key-usage-input-custom-start-date"
                v-model="customStartDate"
                type="date"
                class="input-ring text-xs px-2 py-1.5 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] text-[var(--anthropic-fg)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]"
              />
              <span class="text-xs text-[var(--anthropic-muted)]">-</span>
              <input data-testid="key-usage-input-custom-end-date"
                v-model="customEndDate"
                type="date"
                class="input-ring text-xs px-2 py-1.5 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] text-[var(--anthropic-fg)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]"
              />
              <button data-testid="key-usage-button-query-key-2"
                @click="queryKey"
                class="text-xs px-3 py-1.5 rounded-lg bg-[var(--anthropic-fg)] text-white hover:bg-[var(--anthropic-fg-hover)]"
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-8 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-rose-500'"
              ></span>
              <span class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ statusInfo.label }}</span>
              <span class="text-xs text-[var(--anthropic-muted)] dark:text-dark-500">|</span>
              <span class="text-xs text-[var(--anthropic-muted)] dark:text-dark-400">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-8 transition-all duration-300 hover:shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
              :class="`fade-up-delay-${Math.min(i + 1, 4)}`"
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-[var(--anthropic-muted)] dark:text-dark-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-[var(--anthropic-muted)] dark:text-dark-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-[var(--anthropic-muted)] dark:text-dark-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-[var(--anthropic-muted)] dark:text-dark-400 mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] mt-0.5 tabular-nums">
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] overflow-hidden dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
          >
            <div class="px-8 py-5 border-b border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-dark-800">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-[var(--anthropic-muted)] dark:text-dark-200">{{ row.label }}</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="px-8 py-5 border-b border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-[var(--anthropic-page)] px-6 py-4 dark:bg-[var(--anthropic-section)]"
              >
                <div class="text-xs text-[var(--anthropic-muted)] dark:text-dark-400 mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showDailyUsage"
            class="fade-up fade-up-delay-4 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] backdrop-blur-sm overflow-hidden dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)] sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-0.5 dark:border-[var(--anthropic-border)] dark:bg-dark-950">
                <button data-testid="key-usage-button-set-daily-usage-days-option-value"
                  ...
                  :class="dailyUsageDays === option.value
                    ...
                    : 'text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-raised)] dark:text-dark-300 dark:hover:bg-[var(--anthropic-raised)]'"
                >
                  {{ option.label }}
                </button>
              </div>
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-[var(--anthropic-border)] bg-[var(--anthropic-section)] dark:border-[var(--anthropic-border)] dark:bg-dark-950">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                  ...
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-[var(--anthropic-muted)] dark:text-dark-400">
          ...
          <div
            ...
            class="fade-up fade-up-delay-4 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] overflow-hidden dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
          >
            <div class="px-8 py-5 border-b border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-[var(--anthropic-border)] bg-[var(--anthropic-section)] dark:border-[var(--anthropic-border)] dark:bg-dark-950">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-dark-400">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-[var(--anthropic-border)] last:border-b-0 dark:border-[var(--anthropic-border)]"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ m.model || '-' }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(m.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-[var(--anthropic-muted)] dark:text-dark-200">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-[var(--anthropic-border)] px-6 py-8 dark:border-[var(--anthropic-border)]">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        ...
        <div class="flex items-center gap-4">
          <a data-testid="key-usage-link-a-2"
            v-if="docUrl"
            :href="docUrl"
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-[var(--anthropic-muted)] transition-colors hover:text-[var(--anthropic-muted)] dark:text-dark-400 dark:hover:text-white"
          >{{ t('home.docs') }}</a>
          <a data-testid="key-usage-link-a-3"
            :href="githubUrl"
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-[var(--anthropic-muted)] transition-colors hover:text-[var(--anthropic-muted)] dark:text-dark-400 dark:hover:text-white"
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        rows.push({
          iconBg: 'bg-[var(--anthropic-fg)]', iconColor: 'text-[var(--anthropic-fg)]', iconSvg: ICON_DOLLAR,
    ...
    if (data.subscription) {
      ...
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-accent-800/10', iconColor: 'text-accent-500', iconSvg: ICON_DOLLAR,
