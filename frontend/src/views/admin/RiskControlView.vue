<template>
  <AppLayout>
    <div class="risk-control-atelier space-y-6">
      ...
      <template v-else>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 class="text-2xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.title') }}</h1>
            <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.description') }}</p>
          </div>
          <div class="risk-control-toolbar-actions flex flex-wrap items-center gap-2">
            <button data-testid="admin-risk-control-button-load-status-false" type="button" class="btn btn-primary anthropic-refresh-action-button risk-control-refresh-button" :disabled="statusLoading" @click="loadStatus(false)">
              {{ t('admin.riskControl.refreshStatus') }}
            </button>
            <button data-testid="admin-risk-control-button-open-settings" type="button" class="btn btn-primary risk-control-settings-button" @click="openSettings">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] px-4 py-3 shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center justify-between gap-2">
                  <p class="truncate text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ item.label }}</p>
                  <span
                    v-if="item.badge"
                ...
                <div class="mt-1 flex min-w-0 items-baseline gap-2">
                  <p class="truncate text-xl font-semibold leading-7 text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ item.value }}</p>
                  <p v-if="item.meta" class="truncate text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <div data-test="pre-block-sync-card" class="card">
            <div class="flex flex-col gap-4 border-b border-[var(--anthropic-border)] px-6 py-4 dark:border-[var(--anthropic-border)] lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.preBlockSyncStatus') }}</h2>
                <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.preBlockSyncHint') }}</p>
              </div>
              <span class="inline-flex w-fit items-center rounded-full bg-[var(--anthropic-raised)] px-2.5 py-1 text-xs font-medium text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
            ...
            <div class="p-6">
              <div data-test="pre-block-metric-grid" class="grid grid-cols-2 gap-3 md:grid-cols-3">
                <div
                  ...
                  :class="item.class"
                >
                  <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ item.label }}</p>
                  <p class="mt-2 truncate text-2xl font-semibold leading-8" :class="item.valueClass">{{ item.value }}</p>
                  <p v-if="item.meta" class="mt-1 truncate text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div data-test="pre-block-api-key-load-card" class="card">
            <div class="flex flex-col gap-4 border-b border-[var(--anthropic-border)] px-6 py-4 dark:border-[var(--anthropic-border)] lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.preBlockAPIKeyLoad') }}</h2>
            ...
            <div class="p-6">
              ...
              >
                <div
                  ...
                  class="rounded-lg bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]"
                >
                  <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                      <div class="flex min-w-0 items-center gap-2">
                        <span class="font-mono text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">#{{ item.index + 1 }}</span>
                        <span class="truncate font-mono text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ item.masked || '-' }}</span>
                    ...
                    <div class="grid grid-cols-4 gap-2 text-right text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] sm:min-w-[280px]">
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyActiveShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]">{{ formatNumber(item.active) }}</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyTotalShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(item.total) }}</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyAvgShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(item.avg_latency_ms) }} ms</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyLastShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(item.last_latency_ms) }} ms</p>
                      </div>
                    </div>
                  </div>
                  <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--anthropic-page)] dark:bg-[var(--anthropic-section)]">
                    <div class="h-full rounded-full bg-sky-500" :style="{ width: preBlockAPIKeyLoadWidth(item.total) }"></div>
                  </div>
                </div>
              </div>
              <p v-else class="rounded-lg bg-[var(--anthropic-section)] p-4 text-sm text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
        ...
        <div v-if="showWorkerRuntimeCard" class="card">
          <div class="flex flex-col gap-4 border-b border-[var(--anthropic-border)] px-6 py-4 dark:border-[var(--anthropic-border)] lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.workerStatus') }}</h2>
              <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.workerStatusHint') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              <div class="rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.queueUsage') }}</p>
                    <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      {{ formatNumber(status?.queue_length ?? 0) }} / {{ formatNumber(status?.queue_size ?? configForm.queue_size) }}
                    </p>
                  </div>
                  <span class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ queueUsagePercent }}</span>
                </div>
                <div class="mt-4 h-2 overflow-hidden rounded-full bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
                  <div class="h-full rounded-full bg-[var(--anthropic-fg)] transition-all duration-300" :style="queueUsageStyle"></div>
                </div>
              </div>

              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
                  <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.activeWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ status?.active_workers ?? 0 }}</p>
                </div>
                <div class="rounded-lg bg-emerald-50 p-4 dark:bg-emerald-900/10">
                  <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-emerald-700 dark:text-emerald-300">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
                  <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.processed') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(status?.processed ?? 0) }}</p>
                </div>
                <div class="rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
                  <p class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.droppedErrors') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber((status?.dropped ?? 0) + (status?.errors ?? 0)) }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.workerPool') }}</p>
                  <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                ...
                <span class="inline-flex items-center rounded-full bg-[var(--anthropic-raised)] px-2.5 py-1 text-xs font-medium text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
                  {{ modeLabel(status?.mode ?? configForm.mode) }}
                </span>
              </div>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-[var(--anthropic-border)] px-6 py-4 dark:border-[var(--anthropic-border)]">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.records') }}</h2>
                <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.recordsHint') }}</p>
              </div>
              <button data-testid="admin-risk-control-button-load-logs" type="button" class="btn btn-primary anthropic-refresh-action-button risk-control-logs-refresh-button" :disabled="logsLoading" @click="loadLogs">
                {{ t('admin.riskControl.refresh') }}
              </button>
            </div>

            <div class="flex flex-col gap-2 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] px-3 py-2 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] sm:flex-row sm:items-center sm:justify-between">
              <div class="flex min-w-0 items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                <Icon name="filter" size="sm" class="flex-shrink-0 text-[var(--anthropic-muted)]" />
                <span class="font-medium">{{ t('admin.riskControl.modelFilter') }}</span>
                <span class="truncate text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ modelFilterSummary }}</span>
              </div>
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  v-for="model in modelFilterPreviewModels"
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-md bg-[var(--anthropic-page)] px-2 py-1 font-mono text-xs text-[var(--anthropic-muted)] shadow-none dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-md bg-[var(--anthropic-page)] px-2 py-1 text-xs text-[var(--anthropic-muted)] shadow-none dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
                  +{{ hiddenModelFilterModelCount }}
                </span>
              </div>
            </div>

            <div class="risk-control-record-filters table-filter-shell grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
              <thead class="bg-[var(--anthropic-section)] dark:bg-[var(--anthropic-section)]">
                <tr>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.time') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.group') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.user') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.apiKey') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.endpoint') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.highest') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 bg-[var(--anthropic-page)] dark:divide-dark-800 dark:bg-[var(--anthropic-section)]">
                <tr v-if="logsLoading">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('common.loading') }}</td>
                </tr>
                <tr v-else-if="logs.length === 0">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.emptyLogs') }}</td>
                </tr>
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-[var(--anthropic-section)] dark:hover:bg-[var(--anthropic-raised)]/60">
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ formatDateTime(row.created_at) }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ row.group_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      <div>{{ row.user_email || '-' }}</div>
                      <div v-if="row.user_id" class="text-xs text-[var(--anthropic-muted)]">UID {{ row.user_id }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ row.api_key_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      <div>{{ row.endpoint || '-' }}</div>
                      <div class="text-xs text-[var(--anthropic-muted)]">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      ...
                      <div class="text-xs text-[var(--anthropic-muted)]">{{ percent(row.highest_score) }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      <div>{{ violationCountText(row) }}</div>
                      <div class="text-xs text-[var(--anthropic-muted)]">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      ...
                      <div v-if="row.queue_delay_ms !== null && row.queue_delay_ms !== undefined" class="text-xs text-[var(--anthropic-muted)]">
                        {{ t('admin.riskControl.queueDelay', { ms: row.queue_delay_ms }) }}
                      </div>
                    </td>
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      <button data-testid="admin-risk-control-button-open-input-detail-row"
                        type="button"
                        class="group flex w-full min-w-0 items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-[var(--anthropic-raised)] dark:hover:bg-[var(--anthropic-raised)]"
                        :title="inputSummaryText(row)"
                        @click="openInputDetail(row)"
                      >
                        <span class="min-w-0 flex-1 truncate">{{ inputSummaryText(row) }}</span>
                        <Icon name="eye" size="xs" class="flex-shrink-0 text-gray-300 transition-colors group-hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-muted)]" />
                      </button>
                    </td>
                  </tr>
                </template>
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-[var(--anthropic-border)] pb-3 dark:border-[var(--anthropic-border)]">
            <button data-testid="admin-risk-control-button-active-settings-tab-tab-id"
              ...
              :class="activeSettingsTab === tab.id ? 'bg-[var(--anthropic-section)] text-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]' : 'text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-section)] hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)] dark:hover:text-white'"
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              <div class="flex items-center justify-between rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]">
                <div>
                  <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.enabled') }}</p>
                  <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.enabledHint') }}</p>
                </div>
              ...
              <div>
                ...
                <p class="mt-2 text-xs leading-5 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ modeDescription(configForm.mode) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[var(--anthropic-muted)]">%</span>
                </div>
              </div>
              <div>
            ...
            <div class="overflow-hidden rounded-xl border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <div class="flex flex-col gap-4 border-b border-[var(--anthropic-border)] bg-[var(--anthropic-section)] px-4 py-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-[var(--anthropic-section)] text-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]">
                    <Icon name="key" size="md" />
                  </span>
                  <div>
                    <label class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.apiKeys') }}</label>
                    <p class="mt-1 max-w-3xl text-xs leading-5 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-2 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs leading-5 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                      <span class="font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.apiKeysWriteMode') }}</span>
                      <span class="ml-2">{{ apiKeysModeHint }}</span>
                    </div>
                    <div class="inline-flex rounded-lg bg-[var(--anthropic-page)] p-1 shadow-none dark:bg-[var(--anthropic-section)]">
                      <button data-testid="admin-risk-control-button-set-api-keys-mode-append"
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'append' ? 'bg-[var(--anthropic-fg)] text-white shadow-none' : 'text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-raised)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]'"
                      ...
                      <button data-testid="admin-risk-control-button-set-api-keys-mode-replace"
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-amber-500 text-white shadow-none' : 'text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-raised)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <textarea data-testid="admin-risk-control-textarea-config-form-api-keys-text"
                    ...
                    :disabled="configForm.clear_api_key"
                  ></textarea>
                  <div class="flex flex-wrap items-center gap-2 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                    <span class="inline-flex rounded-md bg-[var(--anthropic-raised)] px-2 py-1 dark:bg-[var(--anthropic-section)]">
                      {{ t('admin.riskControl.inputApiKeyCount', { count: inputApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_key_configured" class="inline-flex rounded-md bg-[var(--anthropic-raised)] px-2 py-1 dark:bg-[var(--anthropic-section)]">
                  ...
                  <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.auditTestInput') }}</p>
                        <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.auditTestInputHint') }}</p>
                      </div>
                      <button data-testid="admin-risk-control-button-clear-moderation-test-input"
                        ...
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-page)] hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)] dark:hover:text-white"
                    ...
                    ></textarea>
                    <div
                      class="mt-3 rounded-lg border border-dashed border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
                      @dragover.prevent
                      @drop.prevent="handleModerationImageDrop"
                    >
                      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-start gap-2">
                          <Icon name="upload" size="md" class="mt-0.5 text-[var(--anthropic-muted)]" />
                          <div>
                            <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.auditTestImages') }}</p>
                            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.auditTestImagesHint') }}</p>
                          </div>
                        </div>
                      ...
                      <div v-if="moderationTestImages.length > 0" class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div
                          ...
                          class="group relative aspect-square overflow-hidden rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-raised)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]"
                        >
                ...
                <div data-test="audit-key-statuses" class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.apiKeyHealth') }}</p>
                      <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.apiKeyFreezeRule') }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-[var(--anthropic-page)] px-2 py-0.5 text-[11px] font-medium leading-5 text-[var(--anthropic-muted)] shadow-none dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
                      {{ t('admin.riskControl.apiKeyRows', { count: apiKeyRows.length }) }}
                    </span>
                  </div>

                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-[var(--anthropic-border)] bg-[var(--anthropic-page)] px-4 py-6 text-center dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
                    <Icon name="infoCircle" size="lg" class="text-gray-300 dark:text-dark-500" />
                    <p class="mt-2 text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.apiKeyHealthEmpty') }}</p>
                    <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.apiKeyHealthEmptyHint') }}</p>
                  </div>
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        class="rounded-lg border bg-[var(--anthropic-page)] p-2.5 shadow-none dark:bg-[var(--anthropic-section)]"
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-amber-200 opacity-70 dark:border-amber-800/60' : 'border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]'"
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              <span class="truncate font-mono text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ row.masked || '-' }}</span>
                              <span
                                class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-medium"
                                :class="row.configured ? 'bg-[var(--anthropic-section)] text-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]' : 'bg-accent-100 text-accent-700 dark:bg-accent-900/30 dark:text-accent-300'"
                              >
                            ...
                            <p class="mt-1 text-xs leading-5 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ apiKeyStatusMeta(row) }}</p>
                          </div>
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            ...
                            <button data-testid="admin-risk-control-button-toggle-delete-stored-api-key-row"
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-[var(--anthropic-muted)] transition-colors hover:bg-[var(--anthropic-raised)] hover:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)] dark:hover:text-gray-200"
                    ...
                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-[var(--anthropic-border)] bg-[var(--anthropic-page)] px-3 py-2 text-xs text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
                      ...
                      <button data-testid="admin-risk-control-button-api-key-rows-expanded-api-key-rows-expanded"
                        type="button"
                        class="inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1 font-medium text-[var(--anthropic-fg)] transition-colors hover:bg-[var(--anthropic-section)] hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)] dark:hover:bg-[var(--anthropic-raised)]"
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
                    ...
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.auditTestResult') }}</p>
                        <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                    ...
                    <div class="mt-3">
                      <div class="mb-2 flex items-center justify-between text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                        <span>{{ t('admin.riskControl.auditTestComposite') }}</span>
                        <span class="font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ percent(moderationTestResult.composite_score) }}</span>
                      </div>
                      <div class="h-2 overflow-hidden rounded-full bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
                    ...
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 text-xs">
                          <span class="truncate text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ score.category }}</span>
                          <span class="font-mono text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ percent(score.score) }} / {{ percent(score.threshold) }}</span>
                        </div>
                        <div class="h-1.5 overflow-hidden rounded-full bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
                          <div class="h-full rounded-full" :class="score.hit ? 'bg-red-500' : 'bg-[var(--anthropic-fg)]'" :style="{ width: percentWidth(score.score) }"></div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.groupScope') }}</h3>
                <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.groupScopeHint') }}</p>
              </div>
              <div class="inline-flex rounded-lg bg-[var(--anthropic-raised)] p-1 dark:bg-[var(--anthropic-section)]">
                <button data-testid="admin-risk-control-button-all-groups-on"
                  type="button"
                  class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors"
                  :class="configForm.all_groups ? 'bg-[var(--anthropic-page)] text-[var(--anthropic-fg)] shadow-none dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]' : 'text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]'"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              <div class="relative">
                <Icon name="search" size="sm" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--anthropic-muted)]" />
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button data-testid="admin-risk-control-button-toggle-group-group-id"
                  ...
                  :class="isGroupSelected(group.id) ? 'border-[var(--anthropic-fg)] bg-[var(--anthropic-section)] dark:border-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)]' : 'border-[var(--anthropic-border)] hover:bg-[var(--anthropic-section)] dark:border-[var(--anthropic-border)] dark:hover:bg-[var(--anthropic-raised)]/60'"
                  @click="toggleGroup(group.id)"
                >
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ group.name }}</span>
                    <span class="mt-1 inline-flex rounded-md bg-[var(--anthropic-raised)] px-2 py-0.5 text-xs text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">{{ group.platform }}</span>
                  </span>
                  <span
                    class="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border"
                    :class="isGroupSelected(group.id) ? 'border-[var(--anthropic-fg)] bg-[var(--anthropic-fg)] text-white' : 'border-[var(--anthropic-border)] text-transparent dark:border-[var(--anthropic-border)]'"
                  >
                    <Icon name="check" size="xs" :stroke-width="2" />
                  </span>
                </button>
                <p v-if="filteredGroups.length === 0" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.noGroups') }}</p>
              </div>
            </div>

            <div class="space-y-4 rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 class="text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.modelFilter') }}</h3>
                  <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.modelFilterHint') }}</p>
                </div>
                <span class="inline-flex w-fit rounded-md bg-[var(--anthropic-raised)] px-2.5 py-1 text-xs font-medium text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
                  {{ modelFilterSummary }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                <button data-testid="admin-risk-control-button-set-model-filter-type-option-value"
                  ...
                  :class="configForm.model_filter_type === option.value
                    ? 'border-[var(--anthropic-fg)] bg-[var(--anthropic-section)] text-[var(--anthropic-fg)] shadow-none dark:border-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]'
                    : 'border-[var(--anthropic-border)] hover:bg-[var(--anthropic-section)] dark:border-[var(--anthropic-border)] dark:hover:bg-[var(--anthropic-raised)]/60'"
                  ...
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      ...
                      :class="configForm.model_filter_type === option.value
                        ...
                        : 'border-[var(--anthropic-border)] text-transparent dark:border-[var(--anthropic-border)]'"
                    >
                      <Icon name="check" size="xs" :stroke-width="2" />
                    </span>
                  </div>
                  <p class="mt-1 text-xs leading-5 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ option.description }}</p>
                </button>
              </div>
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)] lg:col-span-2">
              <div>
                <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.recordNonHits') }}</p>
                <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.recordNonHitsHint') }}</p>
              </div>
              <Toggle v-model="configForm.record_non_hits" />
            </div>
            <div class="space-y-4 rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)] lg:col-span-2">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.preHashCheck') }}</p>
                  <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.preHashCheckHint') }}</p>
                </div>
                <Toggle v-model="configForm.pre_hash_check_enabled" />
              </div>
              <div class="rounded-lg bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.flaggedHashHint') }}</p>
                  </div>
          ...
          <div v-else-if="activeSettingsTab === 'response'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div class="flex items-center justify-between rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]">
                <div>
                  <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.emailOnHit') }}</p>
                  <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.emailOnHitHint') }}</p>
                </div>
                <Toggle v-model="configForm.email_on_hit" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)]">
                <div>
                  <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.autoBan') }}</p>
                  <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.autoBanHint') }}</p>
                </div>
                <Toggle v-model="configForm.auto_ban_enabled" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-[var(--anthropic-border)] p-4 dark:border-[var(--anthropic-border)] lg:col-span-2">
                <div>
                  <p class="text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.cyberPolicyExcludeBan') }}</p>
                  <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.cyberPolicyExcludeBanHint') }}</p>
                </div>
          ...
          <div v-else-if="activeSettingsTab === 'riskThresholds'" class="space-y-5">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.riskThresholds') }}</h3>
                <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.riskThresholdsHint') }}</p>
              </div>
              <button data-testid="admin-risk-control-button-reset-risk-thresholds"
                type="button"
                class="filter-menu-button risk-control-threshold-reset-button"
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <label class="block truncate text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]" :for="`risk-threshold-${row.category}`">
                      {{ row.category }}
                    </label>
                    <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                  ...
                  <span class="inline-flex shrink-0 rounded-md bg-[var(--anthropic-page)] px-2 py-1 font-mono text-xs font-medium text-[var(--anthropic-muted)] shadow-none dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            ...
            >
              ...
              <div class="text-sm leading-6">
                ...
                <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ keywordNotice.description }}</p>
              </div>
            </div>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                ...
                <span class="inline-flex rounded-md bg-[var(--anthropic-raised)] px-2 py-1 text-xs text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-lg border border-[var(--anthropic-border)] p-4 text-sm text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:text-[var(--anthropic-muted)] lg:col-span-2">
              <div class="flex flex-wrap items-center gap-3">
                <Icon name="database" size="md" class="text-[var(--anthropic-muted)]" />
        ...
        <div v-if="inputDetailRow" class="space-y-5">
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <p class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.time') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatDateTime(inputDetailRow.created_at) }}</p>
            </div>
            <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <p class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.user') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ inputDetailRow.user_email || '-' }}</p>
            </div>
            <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <p class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.result') }}</p>
            ...
            <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
              <p class="text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.riskControl.table.highest') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
                {{ inputDetailRow.highest_category || '-' }} / {{ percent(inputDetailRow.highest_score) }}
              </p>
            </div>
          </div>

          <div class="rounded-xl border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-4 shadow-none dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.riskControl.inputDetailContent') }}</p>
                <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                  {{ inputDetailRow.endpoint || '-' }} · {{ inputDetailRow.provider || '-' }} / {{ inputDetailRow.model || '-' }}
                </p>
              </div>
              <span v-if="inputDetailRow.group_name" class="inline-flex rounded-md bg-[var(--anthropic-info-bg)] px-2.5 py-1 text-xs font-medium text-[var(--anthropic-info)] dark:bg-sky-900/20 dark:text-[var(--anthropic-info)]">
...
const keywordNoticeTones = {
  info: {
    ...
    toneClass: 'border-primary-100 bg-[var(--anthropic-section)] dark:border-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)]',
    iconClass: 'mt-0.5 flex-shrink-0 text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]',
    titleClass: 'text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]',
  },
  warning: {
...
const overviewItems = computed<OverviewItem[]>(() => [
  ...
  },
  {
    ...
    icon: 'key',
    iconClass: 'bg-[var(--anthropic-info-bg)] text-[var(--anthropic-info)] dark:bg-sky-900/20 dark:text-[var(--anthropic-info)]',
  },
  {
    key: 'scope',
    ...
    icon: 'users',
    iconClass: 'bg-accent-100 text-accent-600 dark:bg-accent-900/20 dark:text-accent-300',
  },
  {
    key: 'logs',
...
const preBlockMetricItems = computed(() => [
  {
    ...
    class: 'bg-[var(--anthropic-info-bg)] dark:bg-sky-900/10',
    valueClass: 'text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)]',
  },
  {
    key: 'checked',
    ...
    class: 'bg-[var(--anthropic-section)] dark:bg-[var(--anthropic-section)]',
    valueClass: 'text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]',
  },
  {
    key: 'allowed',
  ...
  },
  {
    ...
    class: 'bg-accent-100 dark:bg-accent-900/10',
...
function workerSlotClass(state: WorkerSlotState): string {
  if (state === 'active') {
    return 'border-[var(--anthropic-info-border)] bg-[var(--anthropic-info-bg)] text-[var(--anthropic-info)] dark:border-[var(--anthropic-info-border)] dark:bg-sky-900/20 dark:text-[var(--anthropic-info)]'
  }
  if (state === 'idle') {
  ...
  return 'border-[var(--anthropic-border)] bg-[var(--anthropic-page)] text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]'
}
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ...
    unknown: 'bg-[var(--anthropic-raised)] text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]',
  }
