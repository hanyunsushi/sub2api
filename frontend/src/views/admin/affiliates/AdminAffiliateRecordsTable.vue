<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="table-filter-shell affiliate-records-filter-shell flex flex-wrap items-center gap-3">
          <div class="relative w-full md:w-80">
            <Icon name="search" size="md" class="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--anthropic-muted)]" />
            <input data-testid="admin-affiliates-admin-affiliate-records-table-input-filters-search" v-model="filters.search" type="text" class="input pl-10" :placeholder="t('admin.affiliates.records.searchPlaceholder')" @input="debounceLoad" />
          </div>
          <input data-testid="admin-affiliates-admin-affiliate-records-table-input-filters-start-at" v-model="filters.start_at" type="date" class="input w-full sm:w-44" :title="t('admin.affiliates.records.startAt')" @change="reloadFromFirstPage" />
          <input data-testid="admin-affiliates-admin-affiliate-records-table-input-filters-end-at" v-model="filters.end_at" type="date" class="input w-full sm:w-44" :title="t('admin.affiliates.records.endAt')" @change="reloadFromFirstPage" />
          <button data-testid="admin-affiliates-admin-affiliate-records-table-button-load-records" class="btn btn-primary anthropic-refresh-action-button affiliate-records-refresh-button" :disabled="loading" :title="t('common.refresh')" @click="loadRecords">
            {{ t("common.refresh") }}
          </button>
          <button
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <span v-if="row.order_id == null" class="text-sm text-[var(--anthropic-muted)] dark:text-dark-500">-</span>
            <div v-else class="space-y-0.5">
              <div class="font-mono text-sm text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-[var(--anthropic-muted)] dark:text-dark-400">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span v-if="row.pay_amount == null" class="text-sm text-[var(--anthropic-muted)] dark:text-dark-500">-</span>
            <span v-else class="text-sm text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      <div v-if="overviewLoading" class="flex justify-center py-8">
        <div class="h-6 w-6 animate-spin rounded-full border-2 border-[var(--anthropic-fg)] border-t-transparent"></div>
      </div>
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
          <div class="font-mono text-sm text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-[var(--anthropic-muted)] dark:text-dark-400">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ? 'max-w-56 truncate text-left text-sm font-medium text-[var(--anthropic-fg)] hover:text-[var(--anthropic-fg)] hover:underline dark:text-[var(--anthropic-fg)] dark:hover:text-[var(--anthropic-fg)]'
          : 'max-w-56 truncate text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-[var(--anthropic-muted)] dark:text-dark-400' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ...
        : 'text-sm text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]',
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-[var(--anthropic-muted)] dark:text-dark-500' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]' }, [
      h('div', { class: 'text-sm text-[var(--anthropic-muted)] dark:text-dark-400' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]'
          : 'mt-1 text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]',
      }, statProps.value),
    ])
  },
})
