    ...
    <div class="space-y-6">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 class="text-base font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
            ...
            <p class="mt-1 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              ...
              <button data-testid="admin-backup-button-show-r2-guide-on" type="button" class="text-[var(--anthropic-fg)] underline hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)] dark:hover:text-[var(--anthropic-fg)]" @click="showR2Guide = true">Cloudflare R2</button>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.s3.endpoint') }}</label>
            <input data-testid="admin-backup-input-s3-form-endpoint" v-model="s3Form.endpoint" class="input w-full" placeholder="https://<account_id>.r2.cloudflarestorage.com" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.s3.region') }}</label>
            <input data-testid="admin-backup-input-s3-form-region" v-model="s3Form.region" class="input w-full" placeholder="auto" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.s3.bucket') }}</label>
            <input data-testid="admin-backup-input-s3-form-bucket" v-model="s3Form.bucket" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.s3.prefix') }}</label>
            <input data-testid="admin-backup-input-s3-form-prefix" v-model="s3Form.prefix" class="input w-full" placeholder="backups/" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.s3.accessKeyId') }}</label>
            <input data-testid="admin-backup-input-s3-form-access-key-id" v-model="s3Form.access_key_id" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.s3.secretAccessKey') }}</label>
            <input data-testid="admin-backup-input-s3-form-secret-access-key" v-model="s3Form.secret_access_key" type="password" class="input w-full" :placeholder="s3SecretConfigured ? t('admin.backup.s3.secretConfigured') : ''" />
          </div>
          <label class="inline-flex items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] md:col-span-2">
            <input data-testid="admin-backup-input-s3-form-force-path-style" v-model="s3Form.force_path_style" type="checkbox" />
            <span>{{ t('admin.backup.s3.forcePathStyle') }}</span>
          </label>
        </div>
        <div class="mt-4 flex flex-wrap gap-2">
          <button data-testid="admin-backup-button-test-s3" type="button" class="btn btn-secondary btn-sm" :disabled="testingS3" @click="testS3">
            {{ testingS3 ? t('common.loading') : t('admin.backup.s3.testConnection') }}
          </button>
          <button data-testid="admin-backup-button-save-s3-config" type="button" class="btn btn-primary btn-sm" :disabled="savingS3" @click="saveS3Config">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input data-testid="merge-backup-view-image-storage-form-enabled-1" v-model="imageStorageForm.enabled" type="checkbox" />
            <span>{{ t('admin.backup.imageStorage.enabled') }}</span>
          </label>
        </div>

        <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
          <input data-testid="merge-backup-view-image-storage-form-reuse-backup-s3-2" v-model="imageStorageForm.reuse_backup_s3" type="checkbox" />
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            <input data-testid="merge-backup-view-image-storage-form-bucket-3" v-model="imageStorageForm.bucket" class="input w-full" :placeholder="imageStorageForm.reuse_backup_s3 ? t('admin.backup.imageStorage.bucketInherited') : ''" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.imageStorage.prefix') }}</label>
            <input data-testid="merge-backup-view-image-storage-form-prefix-4" v-model="imageStorageForm.prefix" class="input w-full" placeholder="images/" />
          </div>
          ...
          <template v-if="!imageStorageForm.reuse_backup_s3">
            <div>
              ...
              <input data-testid="merge-backup-view-image-storage-form-endpoint-5" v-model="imageStorageForm.endpoint" class="input w-full" placeholder="https://<account_id>.r2.cloudflarestorage.com" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.region') }}</label>
              <input data-testid="merge-backup-view-image-storage-form-region-6" v-model="imageStorageForm.region" class="input w-full" placeholder="auto" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.accessKeyId') }}</label>
              <input data-testid="merge-backup-view-image-storage-form-access-key-id-7" v-model="imageStorageForm.access_key_id" class="input w-full" />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.backup.s3.secretAccessKey') }}</label>
              <input data-testid="merge-backup-view-image-storage-form-secret-access-key-8" v-model="imageStorageForm.secret_access_key" type="password" class="input w-full" :placeholder="imageStorageSecretConfigured ? t('admin.backup.s3.secretConfigured') : ''" />
            </div>
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 md:col-span-2">
              <input data-testid="merge-backup-view-image-storage-form-force-path-style-9" v-model="imageStorageForm.force_path_style" type="checkbox" />
              <span>{{ t('admin.backup.s3.forcePathStyle') }}</span>
            </label>
          </template>
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <label class="inline-flex items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] md:col-span-2">
            <input data-testid="admin-backup-input-schedule-form-enabled" v-model="scheduleForm.enabled" type="checkbox" />
            <span>{{ t('admin.backup.schedule.enabled') }}</span>
          </label>
          <div>
            <label class="mb-1 block text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.schedule.cronExpr') }}</label>
            <input data-testid="admin-backup-input-schedule-form-cron-expr" v-model="scheduleForm.cron_expr" class="input w-full" placeholder="0 2 * * *" />
            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.schedule.cronHint') }}</p>
          </div>
          <div>
        ...
        <div class="mt-4">
          <button data-testid="admin-backup-button-save-schedule" type="button" class="btn btn-primary btn-sm" :disabled="savingSchedule" @click="saveSchedule">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div class="flex items-center gap-1">
              <label class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.operations.expireDays') }}</label>
              <input data-testid="admin-backup-input-manual-expire-days" v-model.number="manualExpireDays" type="number" min="0" class="input w-20 text-xs" />
            </div>
            <button data-testid="admin-backup-button-create-backup" type="button" class="btn btn-primary btn-sm" :disabled="creatingBackup" @click="createBackup">
              {{ creatingBackup ? t('admin.backup.operations.backing') : t('admin.backup.operations.createBackup') }}
            </button>
            <button data-testid="admin-backup-button-load-backups" type="button" class="btn btn-secondary btn-sm" :disabled="loadingBackups" @click="loadBackups">
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[800px] text-sm">
            <thead>
              <tr class="border-b border-[var(--anthropic-border)] text-left text-xs uppercase tracking-wide text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:text-[var(--anthropic-muted)]">
            ...
            <tbody>
              <tr v-for="record in backups" :key="record.id" class="border-b border-[var(--anthropic-border)] align-top dark:border-[var(--anthropic-border)]">
                ...
                <td class="py-3 text-xs">
                  <div class="flex flex-wrap gap-1">
                    <button data-testid="admin-backup-button-download-backup-record-id"
                    ...
                    <button data-testid="admin-backup-button-restore-backup-record-id"
                      v-if="record.status === 'completed'"
                      type="button"
                      class="btn btn-secondary btn-xs"
                    ...
                    <button data-testid="admin-backup-button-remove-backup-record-id"
                      type="button"
              ...
              <tr v-if="backups.length === 0">
                <td colspan="8" class="py-6 text-center text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
    ...
    <teleport to="body">
      <transition name="modal">
        <div v-if="showR2Guide" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showR2Guide = false">
          <div data-testid="admin-backup-div-show-r2-guide-off" class="fixed inset-0 bg-black/50" @click="showR2Guide = false"></div>
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-[var(--anthropic-page)] p-6 shadow-none dark:bg-[var(--anthropic-section)]">
            <button data-testid="admin-backup-button-show-r2-guide-off" type="button" class="absolute right-4 top-4 text-[var(--anthropic-muted)] hover:text-[var(--anthropic-muted)] dark:hover:text-gray-200" @click="showR2Guide = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.backup.r2Guide.title') }}</h2>
            <p class="mb-4 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.r2Guide.intro') }}</p>
            ...
            <div class="mb-5">
              ...
              <p class="ml-8 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.backup.r2Guide.step3.desc') }}</p>
              <code class="ml-8 mt-1 block rounded bg-[var(--anthropic-raised)] px-3 py-2 text-xs text-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">https://&lt;{{ t('admin.backup.r2Guide.step3.accountId') }}&gt;.r2.cloudflarestorage.com</code>
            </div>

            <!-- Step 4: Fill form -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--anthropic-section)] text-xs font-bold text-[var(--anthropic-fg)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-fg)]">4</span>
              ...
              <div class="ml-8 overflow-hidden rounded-lg border border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in r2ConfigRows" :key="i" class="border-b border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)] last:border-0">
                      <td class="whitespace-nowrap bg-[var(--anthropic-section)] px-3 py-2 font-medium text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">{{ row.field }}</td>
                      <td class="px-3 py-2 text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]"><code class="text-xs">{{ row.value }}</code></td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="mt-4 text-right">
              <button data-testid="admin-backup-button-show-r2-guide-off-2" type="button" class="btn btn-primary btn-sm" @click="showR2Guide = false">{{ t('common.close') }}</button>
            </div>
          </div>
        </div>
      </transition>
