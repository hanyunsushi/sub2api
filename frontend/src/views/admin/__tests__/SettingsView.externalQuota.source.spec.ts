...
describe('SettingsView external quota settings placement', () => {
  ...
  it('collects the XHYAPI user token without asking for a missing refresh_token', () => {
    ...
    expect(source).toContain('localText("XHYAPI 用户令牌", "XHYAPI user token")')
    expect(source).toContain('XHYAPI New API 控制台')
    expect(source).toContain('{id, token} JSON')
    expect(source).toContain('不是 sk- API 密钥')
    expect(source).toContain('https://xhyapi.com')
    expect(source).not.toContain('xhyapi_subscription_refresh_token: string')
    expect(source).not.toContain('form.xhyapi_subscription_refresh_token')
    expect(source).not.toContain('xhyapi_subscription_refresh_token:')
    expect(source).not.toContain('localText("XHYAPI refresh_token", "XHYAPI refresh_token")')
  })

  it('collects the liust user token without asking for a missing refresh_token', () => {
    ...
    expect(source).toContain('localText("liust 用户令牌", "liust user token")')
    expect(source).toContain('liust New API 控制台')
    expect(source).toContain('{id, token} JSON')
    expect(source).toContain('不是 sk- API 密钥')
    expect(source).toContain('https://liust.xyz')
    expect(source).not.toContain('liust_subscription_refresh_token: string')
    expect(source).not.toContain('form.liust_subscription_refresh_token')
    expect(source).not.toContain('liust_subscription_refresh_token:')
    expect(source).not.toContain('localText("liust refresh_token", "liust refresh_token")')
  })
})
