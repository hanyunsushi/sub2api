...
describe('SettingsView external quota settings placement', () => {
  ...
  it('collects the qlhazycoder web login token without asking for a missing refresh_token', () => {
    ...
    expect(source).toContain('localText("qlhazycoder 网页登录 token", "qlhazycoder web login token")')
    expect(source).toContain('localStorage.user')
    expect(source).toContain('user，复制 JSON 里的 token 字段值')
