...
describe('SettingsView external quota settings placement', () => {
  ...
  it('keeps BuzzAI, TCDMX, qlhazycoder, XHYAPI, Pixel, and liust external quota settings in the same gateway card', () => {
    ...
    const pixelIndex = source.indexOf('启用 Pixel 订阅额度')
    ...
    expect(pixelIndex).toBeGreaterThan(xhyapiIndex)
    expect(liustIndex).toBeGreaterThan(pixelIndex)
    expect(liustIndex).toBeLessThan(gatewaySchedulingIndex)
    expect(pixelIndex).toBeLessThan(gatewaySchedulingIndex)
    ...
    expect(source.slice(0, securityEndIndex)).not.toContain('启用 Pixel 订阅额度')
  ...
  it('collects the qlhazycoder user token without asking for a missing refresh_token', () => {
    ...
    expect(source).toContain('qlhazycoder_subscription_user_id')
    ...
    expect(source).toContain('localText("qlhazycoder 用户 ID", "qlhazycoder user ID")')
    expect(source).toContain('个人设置')
    expect(source).toContain('生成/重新生成令牌')
    expect(source).toContain('New-API-User')
  ...
  it('collects the XHYAPI subscription access token and refresh_token like TCDMX', () => {
    ...
    expect(source).toContain('粘贴可访问 XHYAPI 订阅接口的 Token')
    expect(source).toContain('auth_token 过期后自动刷新')
    expect(source).toContain('不是 sk- API 密钥')
    expect(source).toContain('https://xhyapi.com')
    expect(source).not.toContain('localText("XHYAPI 用户令牌", "XHYAPI user token")')
    expect(source).not.toContain('localText("XHYAPI 用户 ID", "XHYAPI user ID")')
    expect(source).not.toContain('XHYAPI New API 控制台')
  })

  it('collects the Pixel subscription access token and refresh_token like TCDMX', () => {
    expect(source).toContain('pixel_subscription_enabled')
    expect(source).toContain('pixel_subscription_api_base_url')
    expect(source).toContain('pixel_subscription_api_token')
    expect(source).toContain('pixel_subscription_api_token_configured')
    expect(source).toContain('pixel_subscription_refresh_token')
    expect(source).toContain('pixel_subscription_refresh_token_configured')
    expect(source).toContain('localText("Pixel 订阅访问 Token", "Pixel subscription access token")')
    expect(source).toContain('localText("Pixel refresh_token", "Pixel refresh_token")')
    expect(source).toContain('粘贴可访问 Pixel 订阅接口的 Token')
    expect(source).toContain('auth_token 过期后自动刷新')
    expect(source).toContain('不是 sk- API 密钥')
    expect(source).toContain('https://ai-pixel.online')
  })
  ...
  it('collects the liust user token without asking for a missing refresh_token', () => {
    ...
    expect(source).toContain('liust_subscription_user_id')
    ...
    expect(source).toContain('localText("liust 用户 ID", "liust user ID")')
