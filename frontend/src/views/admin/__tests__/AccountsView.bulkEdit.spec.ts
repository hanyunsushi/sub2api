  ...
  template: `
    <div data-test="data-table">
      ...
      <div v-for="row in data" :key="row.id">
        <slot name="cell-select" :value="row.select" :row="row" />
...
describe('admin AccountsView bulk edit scope', () => {
  ...
  it('shows account priority beside the card selection checkbox', async () => {
    listAccounts.mockResolvedValue({
      items: [
        {
          id: 7,
    ...
    const priorityBadge = wrapper.get('[data-testid="account-card-priority"]')
    expect(priorityBadge.text()).toContain('P8')
  })
})
