...
<template>
  <div class="anthropic-card-shell p-6">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.runtime.title') }}</h3>
        <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.description') }}</p>
      </div>
    ...
    <div v-if="!alertSettings" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.runtime.alertTitle') }}</h4>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
            ...
            <span class="ml-1 font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ alertSettings.evaluation_interval_seconds }}s</span>
          </div>
          <div
            v-if="alertSettings.silencing?.enabled && alertSettings.silencing.global_until_rfc3339"
            class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] md:col-span-2"
          >
            {{ t('admin.ops.runtime.silencing.globalUntil') }}:
            <span class="ml-1 font-mono text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ alertSettings.silencing.global_until_rfc3339 }}</span>
          </div>

          <details class="col-span-1 md:col-span-2">
            <summary class="cursor-pointer text-xs font-medium text-[var(--anthropic-fg)] hover:text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
              {{ t('admin.ops.runtime.showAdvancedDeveloperSettings') }}
            </summary>
            <div class="mt-2 grid grid-cols-1 gap-3 rounded-lg bg-[var(--anthropic-raised)] p-3 dark:bg-[var(--anthropic-section)] md:grid-cols-2">
              <div class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 font-mono text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 font-mono text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 font-mono text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        ...
        />
        <p class="mt-1 text-xs text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.evalIntervalHint') }}</p>
      </div>

      <div class="rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
        <div class="mb-2 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.runtime.metricThresholds') }}</div>
        <p class="mb-4 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.metricThresholdsHint') }}</p>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input data-testid="admin-ops-components-ops-runtime-settings-card-input-draft-alert-thresholds-sla-percent-min"
              ...
              placeholder="99.5"
            />
            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.slaMinPercentHint') }}</p>
          </div>



          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input data-testid="admin-ops-components-ops-runtime-settings-card-input-draft-alert-thresholds-ttft-p99-ms-max"
              ...
              placeholder="500"
            />
            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.ttftP99MaxMsHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input data-testid="admin-ops-components-ops-runtime-settings-card-input-draft-alert-thresholds-request-error-rate-percent-max"
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.requestErrorRateMaxPercentHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input data-testid="admin-ops-components-ops-runtime-settings-card-input-draft-alert-thresholds-upstream-error-rate-percent-max"
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>

      <div class="rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
        <div class="mb-2 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.runtime.silencing.title') }}</div>

        <label class="inline-flex items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
          <input data-testid="admin-ops-components-ops-runtime-settings-card-input-draft-alert-silencing-enabled" v-model="draftAlert.silencing.enabled" type="checkbox" class="h-4 w-4 rounded border-[var(--anthropic-border)]" />
          <span>{{ t('admin.ops.runtime.silencing.enabled') }}</span>
        </label>

        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            ...
            />
            <p class="mt-1 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.untilHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.reason') }}</div>
          ...
          <div class="rounded-xl border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="text-xs font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.runtime.silencing.entries.title') }}</div>
                <p class="text-[11px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.entries.hint') }}</p>
              </div>
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 rounded-lg bg-[var(--anthropic-section)] p-3 text-xs text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]">
            ...
            <div v-else class="mt-4 space-y-4">
              ...
              >
                <div class="mb-3 flex items-center justify-between">
                  <div class="text-xs font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
      ...
      <details class="rounded-lg border border-[var(--anthropic-border)] bg-[var(--anthropic-section)] p-3 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
        <summary class="cursor-pointer text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="inline-flex items-center gap-2 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              <input data-testid="admin-ops-components-ops-runtime-settings-card-input-draft-alert-distributed-lock-enabled" v-model="draftAlert.distributed_lock.enabled" type="checkbox" class="h-4 w-4 rounded border-[var(--anthropic-border)]" />
              <span>{{ t('admin.ops.runtime.lockEnabled') }}</span>
            </label>
          </div>
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.lockKey') }}</div>
            ...
            <p v-if="draftAlert.distributed_lock.enabled" class="mt-1 text-[11px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              {{ t('admin.ops.runtime.validation.lockKeyHint', { prefix: 'ops:' }) }}
            </p>
          </div>
          <div>
            <div class="mb-1 text-xs font-medium text-[var(--anthropic-muted)]">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
