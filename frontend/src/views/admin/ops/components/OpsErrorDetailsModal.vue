...
<template>
  <BaseDialog :show="show" :title="modalTitle" width="full" @close="close">
    <div class="flex h-full min-h-0 flex-col">
      <!-- Filters -->
      <div class="mb-4 flex-shrink-0 border-b border-[var(--anthropic-border)] pb-4 dark:border-[var(--anthropic-border)]">
        <div class="grid grid-cols-8 gap-2">
          <div class="col-span-2 compact-select">
            <div class="relative group">
              <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <svg
                  class="h-3.5 w-3.5 text-[var(--anthropic-muted)] transition-colors group-focus-within:text-[var(--anthropic-fg)]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <input data-testid="admin-ops-components-ops-error-details-input-q"
                v-model="q"
                type="text"
                class="w-full rounded-lg border-[var(--anthropic-border)] bg-[var(--anthropic-section)] py-1.5 pl-9 pr-3 text-xs font-medium text-[var(--anthropic-muted)] transition-all focus:border-[var(--atelier-focus)] focus:bg-[var(--anthropic-page)] focus:ring-2 focus:ring-[var(--atelier-focus)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:focus:bg-dark-800"
          ...
          <div class="flex items-center justify-end">
            <button data-testid="admin-ops-components-ops-error-details-button-reset-filters" type="button" class="rounded-lg bg-[var(--anthropic-raised)] px-3 py-1.5 text-xs font-semibold text-[var(--anthropic-muted)] transition-colors hover:bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]" @click="resetFilters">
              {{ t('common.reset') }}
            </button>
          </div>
        </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex-shrink-0 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
