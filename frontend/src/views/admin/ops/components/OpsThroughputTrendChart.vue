  ...
  <div class="ops-monitor-panel ops-chart-card ops-throughput-card flex h-full flex-col anthropic-card-shell p-6">
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button data-testid="admin-ops-components-ops-throughput-trend-chart-button-emit-select-group-g-group-id"
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] px-3 py-1 text-[11px] font-semibold text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-section)] dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
