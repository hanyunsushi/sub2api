      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            <svg class="h-8 w-8 animate-spin text-[var(--anthropic-muted)]" fill="none" viewBox="0 0 24 24">
            ...
            <span class="text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-[var(--anthropic-border)] p-10 text-center dark:border-[var(--anthropic-border)]">
            <div class="text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-[var(--anthropic-muted)]">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-gray-200 bg-[var(--anthropic-page)] dark:divide-dark-700 dark:bg-[var(--anthropic-section)]">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-[var(--anthropic-section)] dark:hover:bg-[var(--anthropic-raised)]/50">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button data-testid="admin-ops-components-ops-request-details-button-handle-copy-request-id-row-request-id"
                        class="rounded-md bg-[var(--anthropic-raised)] px-2 py-1 text-[10px] font-bold text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]"
