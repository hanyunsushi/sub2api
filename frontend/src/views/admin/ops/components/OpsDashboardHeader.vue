...
const healthScoreColor = computed(() => {
  ...
  if (isSystemIdle.value) return colors.neutral
...
const healthScoreStyle = computed(() => ({
  color: healthScoreColor.value
}))

const healthScoreRingStyle = computed(() => ({
  color: healthScoreColor.value,
  stroke: healthScoreColor.value,
  '--ops-health-score-color': healthScoreColor.value
}))
...
const cpuPercentClass = computed(() => {
  ...
  if (v == null) return 'text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]'
...
const dbMiddleClass = computed(() => {
  ...
  return 'text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]'
})
...
<template>
  <div :class="['ops-monitor-header flex flex-col gap-4', props.fullscreen ? 'gap-6' : 'gap-4']">
    <!-- Top Toolbar -->
    <div class="ops-monitor-toolbar flex flex-wrap items-center justify-between gap-4">
      <div class="ops-monitor-toolbar-meta">
        <h1 class="text-xl font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
          {{ t('admin.ops.title') }}
        </h1>

        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
      ...
      <div class="ops-monitor-toolbar-controls ml-auto">
        <template v-if="!props.fullscreen">
          <span class="ops-toolbar-labeled-select">
            <span class="ops-toolbar-select-label">来源</span>
            <Select
              ...
              portal-class="ops-toolbar-select-menu"
              @update:model-value="handlePlatformChange"
            />
          </span>

          <span class="ops-toolbar-labeled-select">
            <span class="ops-toolbar-select-label">分组</span>
            <Select
              variant="text-control"
              aria-label="分组"
              :model-value="groupId"
        ...
        <button data-testid="admin-ops-components-ops-dashboard-header-button-handle-toolbar-refresh"
          ...
          type="button"
          class="ops-toolbar-primary-button anthropic-refresh-action-button"
          :disabled="loading"
        ...
        >
          <span>{{ t('admin.ops.fullscreen.enter') }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div class="ops-live-panel lg:col-span-5" aria-label="Health and realtime traffic">
        <div class="ops-live-grid grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="ops-health-score-card relative flex flex-col items-center justify-center rounded-xl"
            :style="healthScoreRingStyle"
          >
            <!-- Diagnosis Popover (hover) -->
            <div
              aria-hidden="true"
              class="ops-diagnosis-popover hidden pointer-events-none absolute left-1/2 top-full z-50 mt-2 w-72 -translate-x-1/2 opacity-0 md:left-full md:top-0 md:ml-2 md:mt-0 md:translate-x-0"
            >
              <div class="ops-diagnosis-popover-panel rounded-xl bg-[var(--anthropic-page)] p-4 shadow-none ring-1 ring-[var(--anthropic-border-subtle)] dark:bg-[var(--anthropic-section)] dark:ring-[var(--anthropic-border-subtle)]">
                <h4 class="mb-3 border-b border-[var(--anthropic-border)] pb-2 text-sm font-bold text-[var(--anthropic-fg)] dark:border-[var(--anthropic-border)] dark:text-[var(--anthropic-fg)] flex items-center gap-2">
                  <Icon name="brain" size="sm" class="text-[var(--anthropic-info)]" />
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      ...
                      <svg v-else-if="item.type === 'warning'" class="h-4 w-4 text-amber-600" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else class="h-4 w-4 text-[var(--anthropic-info)]" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ item.impact }}</div>
                      <div v-if="item.action" class="mt-1 text-[11px] text-[var(--anthropic-info)] dark:text-[var(--anthropic-info)] flex items-center gap-1">
                ...
                <div class="mt-3 border-t border-[var(--anthropic-border)] pt-2 text-[10px] text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)]">
            ...
            <div class="relative flex items-center justify-center">
              <svg
                :width="circleSize"
                :height="circleSize"
                class="ops-health-score-ring -rotate-90 transform"
                :style="healthScoreRingStyle"
              >
                <circle
                  class="ops-health-score-track"
                  :cx="circleSize / 2"
                  :cy="circleSize / 2"
                  :r="radius"
                  ...
                  :style="{ stroke: healthScoreColor }"
                  opacity="0.22"
                />
                <circle
                  class="ops-health-score-progress"
                  :cx="circleSize / 2"
                  :cy="circleSize / 2"
                  :r="radius"
                  :stroke-width="strokeWidth"
              ...
              <div class="absolute flex flex-col items-center">
                <span
                  :class="[props.fullscreen ? 'text-5xl' : 'text-3xl', 'ops-health-score-value font-black']"
                  :style="healthScoreStyle"
                >
                  {{ isSystemIdle ? t('admin.ops.idleStatus') : (overview.health_score ?? '--') }}
                </span>
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-[var(--anthropic-muted)]']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-[var(--anthropic-muted)]">
                {{ t('admin.ops.healthCondition') }}
                <HelpTooltip :content="t('admin.ops.healthHelp')" />
              </div>
              <div class="ops-health-score-state mt-1 text-xs font-bold" :style="healthScoreStyle">
                {{
                  isSystemIdle
          ...
          <div class="ops-realtime-panel flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <div class="relative flex h-3 w-3 shrink-0">
                  <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--anthropic-info)] opacity-75"></span>
                  <span class="relative inline-flex h-3 w-3 rounded-full bg-[var(--anthropic-info)]"></span>
                </div>
                <h3 class="text-xs font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.realtime.title') }}</h3>
                <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.qps')" />
              </div>

              <!-- Time Window Selector -->
              <div class="ops-realtime-window-group flex flex-wrap gap-1">
                <button data-testid="admin-ops-components-ops-dashboard-header-button-realtime-window-window"
                  ...
                  :class="realtimeWindow === window
                    ...
                    : 'bg-[var(--anthropic-raised)] text-[var(--anthropic-muted)] hover:bg-gray-300 dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]'"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-[var(--anthropic-muted)]']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-[var(--anthropic-muted)]']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-[var(--anthropic-muted)]']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-[var(--anthropic-muted)]']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-[var(--anthropic-muted)]']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="ops-metric-card rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-[var(--anthropic-muted)]">{{ t('admin.ops.requestsTitle') }}</span>
            ...
            <button data-testid="admin-ops-components-ops-dashboard-header-button-open-details-title-t-admin-ops-request-details-title"
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-[var(--anthropic-info)] hover:underline"
              type="button"
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="ops-metric-card rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-[var(--anthropic-muted)]">{{ t('admin.ops.sla') }}</span>
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)]">
          ...
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.exceptions') }}:</span>
        ...
        <div class="ops-metric-card rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-[var(--anthropic-muted)]">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-bold text-[var(--anthropic-muted)]">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-[var(--anthropic-muted)]">P95:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-[var(--anthropic-muted)]">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-[var(--anthropic-muted)]">P90:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-[var(--anthropic-muted)]">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-[var(--anthropic-muted)]">P50:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-[var(--anthropic-muted)]">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-[var(--anthropic-muted)]">Avg:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-[var(--anthropic-muted)]">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-[var(--anthropic-muted)]">Max:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-[var(--anthropic-muted)]">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="ops-metric-card rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-[var(--anthropic-muted)]">TTFT</span>
        ...
        <div class="ops-metric-card rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-[var(--anthropic-muted)]">{{ t('admin.ops.requestErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.errors')" />
            </div>
            <button data-testid="admin-ops-components-ops-dashboard-header-button-open-error-details-request" v-if="!props.fullscreen" class="text-[10px] font-bold text-[var(--anthropic-info)] hover:underline" type="button" @click="openErrorDetails('request')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="ops-metric-card rounded-lg bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-[var(--anthropic-muted)]">{{ t('admin.ops.upstreamErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.upstreamErrors')" />
            </div>
            <button data-testid="admin-ops-components-ops-dashboard-header-button-open-error-details-upstream" v-if="!props.fullscreen" class="text-[10px] font-bold text-[var(--anthropic-info)] hover:underline" type="button" @click="openErrorDetails('upstream')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-[var(--anthropic-muted)]">429/529:</span>
              <span class="font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="ops-resource-band mt-2 border-t border-[var(--anthropic-border)] pt-4 dark:border-[var(--anthropic-border)]">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="ops-system-card rounded-xl bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
        ...
        <div class="ops-system-card rounded-xl bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="ops-system-card rounded-xl bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.db') }}</div>
        ...
        <div class="ops-system-card rounded-xl bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">Redis</div>
        ...
        <div class="ops-system-card rounded-xl bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="ops-system-card rounded-xl bg-[var(--anthropic-section)] p-3 dark:bg-[var(--anthropic-section)]">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.jobs') }}</div>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.jobs')" />
            </div>
            <button data-testid="admin-ops-components-ops-dashboard-header-button-open-jobs-details" v-if="!props.fullscreen" class="text-[10px] font-bold text-[var(--anthropic-info)] hover:underline" type="button" @click="openJobsDetails">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ hb.job_name }}</div>
            <div class="flex items-center gap-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              <span v-if="hb.last_duration_ms != null" class="font-mono">{{ hb.last_duration_ms }}ms</span>
              <span>{{ formatTimeShort(hb.updated_at) }}</span>
            </div>
          </div>

          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] sm:grid-cols-2">
            <div>
    ...
    <BaseDialog :show="showCustomTimeRangeDialog" :title="t('admin.ops.timeRange.custom')" width="narrow" @close="handleCustomTimeRangeCancel">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)] mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button data-testid="admin-ops-components-ops-dashboard-header-button-handle-custom-time-range-cancel"
            type="button"
            class="rounded-lg bg-[var(--anthropic-raised)] px-4 py-2 text-sm font-medium text-[var(--anthropic-muted)] hover:bg-[var(--anthropic-raised)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:hover:bg-[var(--anthropic-raised)]"
