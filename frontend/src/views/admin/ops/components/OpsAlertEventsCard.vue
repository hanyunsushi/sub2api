...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p2' || s === 'info') return 'bg-[var(--anthropic-info-bg)] text-[var(--anthropic-info)] dark:bg-[var(--anthropic-info-bg)] dark:text-[var(--anthropic-info)]'
  if (s === 'p3') return 'bg-[var(--anthropic-raised)] text-[var(--anthropic-muted)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)]'
...
function statusBadgeClass(status: string | undefined): string {
  ...
  if (s === 'manual_resolved') return 'bg-[var(--anthropic-section)] text-slate-700 ring-slate-600/20 dark:bg-[var(--anthropic-section)] dark:text-slate-300 dark:ring-slate-500/30'
...
<template>
  <div class="ops-monitor-panel ops-alert-card anthropic-card-shell p-6">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.alertEvents.title') }}</h3>
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-[var(--anthropic-border)] p-8 text-center text-sm text-[var(--anthropic-muted)] dark:border-[var(--anthropic-border)] dark:text-[var(--anthropic-muted)]">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-gray-200 bg-[var(--anthropic-page)] dark:divide-dark-700 dark:bg-[var(--anthropic-section)]">
            <tr data-testid="admin-ops-components-ops-alert-events-card-tr-open-detail-row"
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-[var(--anthropic-section)] dark:hover:bg-[var(--anthropic-raised)]/50"
            ...
            >
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  />
                  <span class="text-[11px] font-bold text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-[var(--anthropic-muted)]">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-[var(--anthropic-page)] px-2 py-1 ring-1 ring-gray-200 dark:bg-[var(--anthropic-section)] dark:ring-dark-700">
                <span class="text-[11px] font-bold text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
              <div class="text-xs font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
              <div class="text-xs font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
              <div class="text-xs font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">#{{ selected.rule_id }}</div>
                <a data-testid="admin-ops-components-ops-alert-events-card-link-a"
                  class="inline-flex items-center gap-1 rounded-md bg-[var(--anthropic-page)] px-2 py-1 text-[11px] font-bold text-[var(--anthropic-muted)] ring-1 ring-gray-200 hover:bg-[var(--anthropic-section)] dark:bg-[var(--anthropic-section)] dark:text-[var(--anthropic-muted)] dark:ring-dark-700 dark:hover:bg-[var(--anthropic-raised)]"
            ...
            <div class="rounded-xl bg-[var(--anthropic-section)] p-4 dark:bg-[var(--anthropic-section)]">
              <div class="text-xs font-bold uppercase tracking-wider text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
              <div class="mt-1 text-sm text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">
        ...
        <div class="rounded-xl border border-[var(--anthropic-border)] bg-[var(--anthropic-page)] p-4 dark:border-[var(--anthropic-border)] dark:bg-[var(--anthropic-section)]">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-[var(--anthropic-border)] dark:border-[var(--anthropic-border)]">
            <table class="min-w-full divide-y divide-gray-100 dark:divide-dark-700">
              <thead class="bg-[var(--anthropic-section)] dark:bg-[var(--anthropic-section)]">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-[var(--anthropic-section)] dark:hover:bg-[var(--anthropic-raised)]/50">
                  <td class="px-3 py-2 text-xs text-[var(--anthropic-muted)] dark:text-[var(--anthropic-muted)]">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
