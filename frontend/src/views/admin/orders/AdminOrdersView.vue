      ...
      <div class="card p-4">
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            <button data-testid="admin-orders-admin-orders-button-load-orders" @click="loadOrders" :disabled="ordersLoading" class="btn btn-secondary" :title="t('common.refresh')">
      ...
      <OrderTable :orders="orders" :loading="ordersLoading" show-user>
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            <button data-testid="admin-orders-admin-orders-button-show-order-detail-row" @click="showOrderDetail(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-dark-600">
            ...
            <button data-testid="admin-orders-admin-orders-button-handle-retry-order-row" v-if="row.status === 'FAILED'" @click="handleRetryOrder(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              ...
              <button data-testid="admin-orders-admin-orders-button-open-refund-dialog-row" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20">
                <Icon name="check" size="sm" />
                {{ t('payment.admin.approveRefund') }}
              </button>
            </template>
            <button data-testid="admin-orders-admin-orders-button-open-refund-dialog-row-2" v-else-if="row.status === 'REFUND_FAILED'" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20">
              <Icon name="refresh" size="sm" />
