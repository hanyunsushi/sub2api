    ...
    <div class="space-y-4">
      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-price="{ value, row }">
          <div class="text-sm">
            <span class="font-medium text-[var(--anthropic-fg)] dark:text-[var(--anthropic-fg)]">{{ planCurrencySymbol(row.currency) }}{{ (value ?? 0).toFixed(2) }}</span>
            <span v-if="row.currency" class="ml-1 text-xs text-[var(--anthropic-muted)]">{{ row.currency }}</span>
            <span v-if="row.original_price" class="ml-1 text-xs text-[var(--anthropic-muted)] line-through">{{ planCurrencySymbol(row.currency) }}{{ row.original_price.toFixed(2) }}</span>
          </div>
        </template>
