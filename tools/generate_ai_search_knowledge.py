...
def build_markdown(source: str) -> str:
    ...
    if source_has(source, "Cloudflare AI Search", "AI Search"):
        lines.extend(
            [
                "## AI Search",
                "",
                "登录后的网页控制台右上角有 `AI Search` 搜索框，位置在公告铃左侧。用户可以直接搜索常见问题、功能说明、备份说明和账号管理说明。",
                "浏览器只请求 Sub2API 后端接口，后端再查询 Cloudflare AI Search；Cloudflare 凭据不会暴露给前端。",
                "实例名称使用 `ai-search`，界面文案使用 `AI Search`，不使用 Help 作为名称。",
                "",
            ]
        )
    ...
    lines.extend(
        [
            ...
            "## FAQ 如何累积",
            "",
            "适合公开给用户搜索的 FAQ、功能说明和操作说明应先写入受控文档，再同步到 Cloudflare AI Search。",
            ...
            "",
            "可以。用户登录后可以在右上角 `AI Search` 搜索框中查询网站功能、账号管理、备份范围和常见问题。",
            "",
            "### 整份运维文档能直接作为知识库吗？",
            "",
