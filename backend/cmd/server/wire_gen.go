...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, accountRepository, settingService)
	...
	externalSubscriptionConfigService := service.NewExternalSubscriptionConfigService(settingService)
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, externalSubscriptionConfigService)
	...
	aiSearchConfigService := service.NewAISearchConfigService(settingRepository, configConfig, secretEncryptor)
	aiSearchService := service.ProvideAISearchService(configConfig, aiSearchConfigService)
	aiSearchKnowledgeSyncService := service.ProvideAISearchKnowledgeSyncService(configConfig, aiSearchConfigService)
	backupHandler := admin.NewBackupHandler(backupService, userService, aiSearchConfigService, aiSearchService, aiSearchKnowledgeSyncService)
	...
	codexMetadataRepository := repository.NewCodexMetadataRepository(client)
	codexMetadataService := service.NewCodexMetadataService(codexMetadataRepository)
	codexMetadataHandler := admin.NewCodexMetadataHandler(codexMetadataService)
	buzzBalanceService := service.NewBuzzBalanceService(settingService)
	buzzBalanceHandler := admin.NewBuzzBalanceHandler(buzzBalanceService)
	externalSubscriptionConfigHandler := admin.NewExternalSubscriptionConfigHandler(externalSubscriptionConfigService)
	tcdmxSubscriptionService := service.NewTCDMXSubscriptionService(settingService)
	tcdmxSubscriptionHandler := admin.NewTCDMXSubscriptionHandler(tcdmxSubscriptionService)
	qlHazyCoderSubscriptionService := service.NewQLHazyCoderSubscriptionService(settingService)
	qlHazyCoderSubscriptionHandler := admin.NewQLHazyCoderSubscriptionHandler(qlHazyCoderSubscriptionService)
	xhyapiSubscriptionService := service.NewXHYAPISubscriptionService(settingService)
	xhyapiSubscriptionHandler := admin.NewXHYAPISubscriptionHandler(xhyapiSubscriptionService)
	pixelSubscriptionService := service.NewPixelSubscriptionService(settingService)
	pixelSubscriptionHandler := admin.NewPixelSubscriptionHandler(pixelSubscriptionService)
	liustSubscriptionService := service.NewLiustSubscriptionService(settingService)
	liustSubscriptionHandler := admin.NewLiustSubscriptionHandler(liustSubscriptionService)
	packyCodeSubscriptionService := service.NewPackyCodeSubscriptionService(settingService)
	packyCodeSubscriptionHandler := admin.NewPackyCodeSubscriptionHandler(packyCodeSubscriptionService)
	complianceHandler := admin.NewComplianceHandler(settingService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, codexMetadataHandler, buzzBalanceHandler, externalSubscriptionConfigHandler, tcdmxSubscriptionHandler, qlHazyCoderSubscriptionHandler, xhyapiSubscriptionHandler, pixelSubscriptionHandler, liustSubscriptionHandler, packyCodeSubscriptionHandler, complianceHandler)
	...
	globalPricingHandler := handler.ProvideGlobalPricingHandler(pricingService)
	aiSearchHandler := handler.ProvideAISearchHandler(aiSearchService, aiSearchConfigService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, globalPricingHandler, aiSearchHandler, idempotencyCoordinator, idempotencyCleanupService)
