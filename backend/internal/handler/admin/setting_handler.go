...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.CustomMenuItems != nil {
		...
		for i, item := range items {
			...
			case "redirect":
				if strings.HasPrefix(urlTrimmed, "md:") {
					response.BadRequest(c, "Custom menu item markdown pages must use iframe open mode")
					return
				}
			default:
				response.BadRequest(c, "Custom menu item open_mode must be 'iframe' or 'redirect'")
				return
			}
...
func (h *SettingHandler) SendTestEmail(c *gin.Context) {
	...
	body := service.BuildAnthropicEmailBody(siteName, "Email configuration successful", `
<div class="email-block">
  <h2>SMTP connection is ready.</h2>
  <p>This is a test email to verify your SMTP settings are working correctly.</p>
</div>
<p class="muted">This is an automated test message.</p>
`)
