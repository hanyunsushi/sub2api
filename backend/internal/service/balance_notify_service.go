...
func (s *BalanceNotifyService) buildBalanceLowEmailBody(userName string, balance, threshold float64, siteName, rechargeURL string) string {
	...
	content := fmt.Sprintf(`
<p>%s，您的余额不足。</p>
<p>Dear %s, your balance is running low.</p>
<div class="email-block email-warning">
  <h2>$%.2f</h2>
...
%s`,
	...
	return buildAnthropicEmailBodyRaw(siteName, "余额不足提醒", content)
}
...
<div class="email-block email-danger">
  <table class="email-table">
	...
	return buildAnthropicEmailBodyRaw(siteName, "账号限额告警", content)
}
